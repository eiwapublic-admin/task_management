...
//
// kind: 'inspection' | 'chlorine' | 'equipment'。保存先はユーザーごとに固定キー（upsert）だが、
...
// 3施設目（当時の呼称「スイングビル」）を対象外にしていたが、FileMaker側では
// 直近まで継続して測定記録が入力されていたことが移行時（2026-08-25）に判明し、
// 3施設目として追加した。その後、施設の呼称が「旧本社」に変更されたため
// （2026-08-25）、表記・既存データともに「旧本社」に統一している
export const CHLORINE_BUILDINGS = ['BKB', '小泉本社', '旧本社']
