...
export async function handleDocumentPreviewToken(req) {
  ...
  try {
    ...
    const token = await signJwt({ purpose: 'document-preview', id }, SESSION_SECRET, DOCUMENT_PREVIEW_TTL_SECONDS)
    return json({ token })
  } catch (err) {
...
// GET /api/documents/download?id=…[&preview_token=…] — ファイル本体を返す。
// preview_token（上記handleDocumentPreviewTokenが発行）が有効かつ対象のidと
...
export async function handleDocumentDownload(req) {
  ...
  if (previewToken) {
    ...
    viaPreviewToken = !!claims && claims.purpose === 'document-preview' && claims.id === id
  }
  if (!viaPreviewToken) {
    const { error } = await requireAuth(req)
    if (error) return error
