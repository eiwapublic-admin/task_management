...
最終更新: 2026-08-11（計画のみ。実装は未着手。**確認事項12件のうち9件に回答をいただき反映済み**
＝12-1。残るは⑦owner表示・⑪各種出力・⑫CSV列名の3件で、いずれも着手を止めるものではない。
特に③の回答により「**テナント欄＝請求先**」であることが判明し、3-4 として設計原則に格上げした）
...
| DB（4-1） | `tenant_code` / `tenant_name` を持てるのは `reason='tenant'` のときだけ、という制約を付ける |
| 外部API（6-2） | 請求の元データになるため、**新規入替・不良品処分は一切返さない**。`scope=tenant` は `reason='tenant'` のみ |
| 移行（8章） | CSV 取込時も同じ制約を通す。現行データに「新規入替なのにテナント名が入っている」行があれば、それは**請求対象外なのに請求先が入っている**ことになるので、取り込まずに一覧化して確認いただく |

**なぜ制約で縛るか**: 誤ってテナントを選んだまま新規入替で保存すると、**本来請求しないものを
請求してしまう**（＝金銭的な影響が出る）。画面の出し分けだけに頼ると、後から入力区分を変えたとき
などに値が残る事故が起こりうるため、DB とAPIの両方で止める。
...
create table if not exists equipment_transactions (
  ...
  -- 請求先（テナント）を持てるのはテナント設置のときだけ。
  -- テナント欄は場所のラベルではなく「そのテナントに請求する」という意味を持つため、
  -- 請求対象外の新規入替・共用部設置・不良品処分に値が残らないよう DB 側でも止める（3-4）
  constraint equipment_txn_tenant_only_tenant_reason check (
    (tenant_code is null and tenant_name is null) or reason = 'tenant'
  )
);
...
> 同じ項目」と仮置きしていたが、実際の運用では**備考しか入力しない**。
>
...
「その他は後々用意します」との回答（2026-08-11）。着手を止めるものではないため、
⑦は下記の推奨で進め、⑪⑫は資料をいただいた時点で対応する。

| # | 内容 | こちらの推奨・扱い |
|---|---|---|
| ⑦ | **owner（小泉産業様）に備品セクションを見せるか** | **見せない前提で実装する**（社内の在庫管理のため）。見せることになっても、日報と同じ「閲覧のみ」を足すだけで済むので後から変更できる |
| ⑪ | 「**各種出力**」で現在どんな帳票を出しているか | 今回は範囲外。必要になった時点で要件を伺って別途 |
| ⑫ | CSV の**列名・件数**（8-1 の想定と合っているか） | 実ファイルをいただいた時点でこちらで合わせる（Phase 0） |
