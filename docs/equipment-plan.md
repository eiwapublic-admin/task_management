...
最終更新: 2026-08-11（計画のみ。実装は未着手。**確認事項17件すべてに回答をいただき反映済み**＝12-1。
移行用CSV4本（全期間の入出庫830件）と帳票2点の実物を受領し、全件を機械検証した結果を
8-1・8-4・8-5 に反映。**帳票（修理伝票PDF・修理データ）を範囲に追加**し、owner にも
修理伝票PDFを含めて閲覧を開放する形に変更した。テナント欄＝請求先という業務ルール（3-4）は、
過去データに例外があったため DB の CHECK 制約ではなくアプリ層のバリデーションに変更した。
残る判断は⑱（新規入替で階・場所欄を残すか）のみ＝12-2）
...
- **新規入力**（今後）: ご指示どおり備考のみ（5-4）。設置先を残したい場合の階・場所欄については
  12-2 ⑱で改めて確認する
...
| # | 内容 | こちらの推奨 |
|---|---|---|
| ⑱ | 新規入替は今後の入力も「備考のみ」で確定したが、実データでは過去に階・設置先も使われていた（8-4(1)(2)）。**今後、「どこに入れ替えたか」を集計できる形（階・場所欄）で残したい場面はあるか**、それとも備考の自由記述で十分か | 備考のみで進める（ご指示どおり）。後日「やはり階・場所も分けて残したい」となった場合、`floor`/`location` 列は既に共用部設置用に存在するため、**新規入替でも同じ欄を使う形に画面だけ広げれば足りる**（DB変更は不要） |
