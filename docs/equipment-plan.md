...
3. テナント設置時は、**その場で iPad / スマホに手書き署名**をもらい、受領の証跡として保存する
...
- 実装上の注意点（先に潰しておく）:
  ...
  6. **未署名のまま保存させない**（テナント設置時）。`取消`（消して書き直し）と `閉じる` を分ける
  7. 横向き（landscape）でも枠が潰れないよう、高さは `max-height: 100%` の枠内で比率固定にする
- 保存フロー: 署名 PNG は**入出庫レコードの保存と同時**に送る（multipart）。
  レコードだけ先に作って署名が付かない状態を作らない。署名が付いた時刻を `signed_at` に入れる
...
| POST `/api/equipment/transactions` | 入出庫の登録。`multipart/form-data`（署名 PNG を同送）。`kind`/`reason` の整合はサーバー側でも検証 |
...
| Phase | 内容 | 前提 | 目安 |
|---|---|---|---|
| **0. 準備** | 12章の確認事項の回答、CSV 4本の受領、FileMaker 側の到達性方針の決定 | — | 打ち合わせのみ |
| **1. 基盤＋在庫と入出庫（共用部まで）** | DB・ビュー / ヘッダに「備品」追加 / 在庫一覧 / 履歴 / 入庫モーダル / 出庫（共用部・不良品処分） / 備品・カテゴリマスタ / CSV 取込スクリプト | Phase 0 の CSV | 1〜2セッション |
| **2. テナント設置＋署名** | テナント一覧（CSV 取込分） / 出庫（テナント設置・新規入替） / **署名キャプチャ** / 署名の表示・保管 | Phase 1 | 1〜2セッション |
...
## 12. 確認事項（着手前に回答をいただきたい）

| # | 内容 | こちらの推奨 |
|---|---|---|
| ① | **FileMaker Server の外部到達性**をどうするか（7-1 の A/B/C） | **A. Cloudflare Tunnel**。ポートを開けずに済む。難しければ C（Push）も可 |
| ② | 「**新規入替**」「**繰延登録**」は実際にどう使い分けているか。入力項目は何が要るか | 新規入替＝テナント設置と同じ項目（署名は任意）、繰延登録＝調達と同じ項目、と仮置きして進める |
| ③ | 「新規入替」に設置先（テナント／共用部）の入力は要るか | 要る前提（テナント選択を出す）で仮置き |
| ④ | テナント設置で**署名は必須か**（もらえない場合の運用があるか） | 必須にし、例外時は「署名なしで記録」を備考必須で許す形を推奨 |
| ⑤ | **過去の署名画像**は移行するか | 移行しない（CSV に載らない）。過去分は現行 FileMaker を参照用に残す |
| ⑥ | 入出庫の過去データは**全期間**移行するか、直近のみか | 全期間（年計が過去年も正しく出る。データ量は問題にならない） |
