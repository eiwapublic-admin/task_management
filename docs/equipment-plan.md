...
[ 在庫 ]                                        [ 入庫 ] [ 出庫 ]
  ┌────────────────────────────────────────┐
  │ A4 蛍光灯 40                            │  ← カテゴリ見出し
  │  FLR40SW          [出][入]           44 │  ← 行タップで履歴へ
  │  FLR40EX          [出][入]            0 │  ← 赤バッジ＋「発注依頼」
  └────────────────────────────────────────┘
```
...
- **行の出/入ボタン（2026-08-18。現行FileMakerの「入出庫記録」画面が備品ごとの見出し帯に
  入庫/出庫ボタンを持っていたのと同じ発想を復活させた）**: 在庫数の左に小さな「出」「入」ボタン
  （`.equipment-stock-action-btn`）を置き、押すとその備品を選んだ状態で出庫・入庫モーダルを開く
  （`EquipmentOutForm`/`EquipmentInForm`の新しい`defaultItemId` prop）。iPhone幅で1行に収めるため
  キャプションは1文字（「出」「入」）に省略している。owner（読み取り専用ロール）には表示しない
