...
| GET `/api/equipment/tenants` | テナント一覧（`include_moved_out=1` で退去済みも。`synced_at`/`missing_since` を含み「最終同期」表示に使う） |
...
### 6-3. FileMaker からのテナント同期受信（APIキー認証・書き込み専用）← 【2026-08-12 新設】
...
Body: { "billing_code": "...", "name": "...", "short_name": "...", "floor": "...", "moved_out": false, "note": "..." }
      または上記オブジェクトの配列（全件送信用）
```

セキュリティ設計:

1. **6-2 とは独立したキー**で、6-2 のキーではこのエンドポイントを叩けない
2. `billing_code` を突合キーに **upsert のみ**。`DELETE` は生やさない（退去は `moved_out` フラグで表現。7-1）
3. 送信元 IP の許可リスト（`EQUIPMENT_API_ALLOW_IPS` を共用）、KV でのレート制限（6-2 と同じ仕組み）
4. 呼び出しは `activity_logs` に1行残す
5. 配列送信（全件送信）は上限件数（例: 200件）を設け、超える場合はエラーで弾く（誤操作対策）

FileMaker 側の呼び出し例（`Insert from URL`、保存時スクリプトトリガーから）:

```
"-X POST
--header " & Quote("X-API-Key: ***") & "
--header " & Quote("Content-Type: application/json") & "
--data " & Quote(JSONオブジェクト)
```
...
POST https://<worker-host>/api/equipment/tenants/sync
  ...
  Body: テナント1件、または配列（請求先コード・正式名称・略称・階・退去済み・備考）
```

**同期の作り**:
- **トリガーは FileMaker 側のレイアウト保存時スクリプト**（テナントの追加・改称・退去フラグ変更などで
  レコードが保存されるたびに、その1件を JSON にして `Insert from URL` で送信）
- Worker 側は `billing_code` を突合キーに **upsert**（`billing_code` が空のテナントは新規作成のみ扱い、
  名寄せでの上書きはしない。空欄テナントの修正は従来どおりテナント一覧画面から手で行う）
- `default_item_id` は当システム側でのみ持つ項目のため、送信内容で**上書きしない**
- **退去は行の削除ではなく「退去済み」フラグの送信で表現する**（4-1 の `moved_out`。CSV 移行時と同じ考え方）。
  FileMaker 側でレコードそのものを削除した場合は、個別 Push だけでは検知できない
- **そのため、週1回 or 手動実行の「全件送信」スクリプトを別途用意する**。テナント一覧を配列でまとめて
  送る同期で、Worker 側は結果に無くなった行を**消さずに** `missing_since` を立てるだけにする
  （過去の入出庫が参照しているため。個別 Push だけに頼らず、取りこぼしをここで拾う）
- **FileMaker が落ちていても現場の入力は止まらない**。当システムは受信済みのコピーで動く。
  送信失敗は Push の性質上 Worker 側からは検知できないため、テナント一覧画面の「最終同期」表示と
  週次の全件送信が実質的な監視になる
...
| # | 内容 | 誰が |
|---|---|---|
| 1 | テナントレイアウトに**保存時スクリプトトリガー**を設定し、`Insert from URL` で送信するスクリプトを作成 | FileMaker 管理者 |
| 2 | **週1回（or 手動）の全件送信スクリプト**を作成（配列送信。削除の取りこぼしを拾う） | FileMaker 管理者 |
| 3 | 当方が払い出す送信用 APIキーを、両スクリプトの `cURL` オプションに設定 | 両者 |
...
| **5. テナントマスタ同期** | 7-1 の受信用エンドポイント（Push） / 保存時トリガー・全件送信スクリプトの提示 | **FileMaker 側の準備（7-1 の表 1〜3）** | 1セッション＋先方作業 |
...
| リスク | 影響 | 対策 |
|---|---|---|
| **テナント同期の取りこぼし**（7-1・Push方式） | FileMaker 側でレコードが削除された場合、個別 Push だけでは Worker が検知できない | 週1回 or 手動の「全件送信」スクリプトを併設し、結果に無い行は `missing_since` を立てて拾う（2026-08-12、Pull→Push化に伴い旧リスク「FileMaker Server の常時公開」は解消） |
