...
最終更新: 2026-08-11（計画のみ。実装は未着手。**当初の確認事項12件すべてに回答をいただき反映済み**＝12-1。
移行用CSV4本と帳票2点の実物を受領し、全件を機械検証した結果を 8-1・8-4・8-5 に反映。
**帳票（修理伝票PDF・修理データ）を範囲に追加**し、owner にも閲覧を開放する形に変更した。
検証で当初の想定と食い違う点が5つ見つかったため、**新たに⑬〜⑰のご判断をお願いしたい**＝12-2）
...
create table if not exists equipment_transactions (
  ...
  -- ※ 移行データに1件だけ違反行がある（8-4(1)）。取込時に請求先を落として通す想定
  constraint equipment_txn_tenant_only_tenant_reason check (
    (tenant_id is null and tenant_code is null and tenant_name is null) or reason = 'tenant'
  )
);
...
| 入出庫データ | **30** | 入出荷ID / 入出庫区分 / 入出庫日付 / 入出庫時刻 / 入出庫理由 / 調達先 / **設置先請求先コード** / 設置階 / 設置先 / 備品ID / 入庫数量 / 出庫数量 / 担当者 / 備考 / **受領タイムスタンプ** / 主キー / 作成・修正情報（ほか集計用の `c_*` 列多数） |
...
**検証で見つかった実データの状態**（→ 8-5 に詳述）:

| 確認 | 結果 |
|---|---|
| 入出庫の請求先コードがテナントマスタに存在するか | **7件すべて一致**（欠損なし） |
| 入出庫の `設置先` がテナントの `略称` と一致するか | **テナント設置は全件一致** |
| テナント設置の `受領タイムスタンプ` は作成時刻と別か | **6件すべて別**（数分後）＝**署名を受け取った時刻として使える** |
| 入庫の `調達先` | **7件すべて空**（実運用では使われていない） |
| 請求先コードが空のテナント | **2件あり**（どちらも退去済み）→ 主キー設計に影響 |
| 新規入替に請求先コードが入っている行 | **1件あり**（3-4 の禁止事項に該当）→ 要判断 |

- **過去の署名画像は移行しない**（CSV に載らない。現行 FileMaker を参照用に残す）。**2026-08-11 に「移行なしでよい」と確定**（確認事項⑤）
- ⚠ **入出庫データが 30 件しかない。** 日付は 2023/02/20〜2026/08/10 に散らばっており、
  現行画面の「年計（入庫52/出庫8）」とも合わないため、**全件ではなく抜粋と考えられる**。
  「全件移行可能」（確認事項⑥）とのことなので、**全期間の再書き出しをお願いしたい**（→ 12-2）
...
> ただし受領した入出庫データは 30 件しかなく抜粋と思われるため、**全期間の再書き出しが必要**（8-1）。

### 8-4. 移行時に判断が要る点（実データの検証で判明。2026-08-11）

いずれも**取込スクリプトを書く前に決めておかないと、後から直すのが高くつく**もの。

#### (1) 新規入替なのに請求先コードが入っている行が実在する ← 最重要

3-4 で「新規入替に請求先（テナント）は持たせない」と決めたが、**実データに該当行があった**。
...
備考から見て**入居準備＝請求対象外**の作業であり、請求先コードが入っているのは記録上の綾と思われる。
このまま取り込むと 3-4 の制約に反する（DB が受け付けない）。**方針をご判断いただきたい**（→ 12-2 ⑬）。

- **推奨: 請求先コードを落として取り込む**（設置階・設置先・備考はそのまま残す）。
  請求実績（8-5 の修理データ）にも元々載っていないため、落としても請求の履歴は変わらない
- 代案: この行だけ理由を「テナント設置」に直して取り込む（＝請求対象として扱う）。
  ただし過去の請求と食い違うことになるので推奨しない

#### (2) 新規入替でも「設置階・設置先」は使われている

「新規入替の入力は備考のみ」（確認事項②）とのご回答だったが、**実データでは 2件とも設置階・設置先に
値が入っていた**（`1F / 喫煙室`、`1F / 小泉情報`）。備考を使っていたのは片方だけ。

- **移行**: 過去分は情報を捨てないよう、**設置階・設置先をそのまま `floor` / `location` に取り込む**
- **新規入力**: ご指示どおり備考のみとする（5-4）。
  ただし「どこに入れ替えたか」を今後も残したいなら、共用部設置と同じ階・場所欄を出す方が自然
  （備考への自由記述だと後から集計できない）→ 12-2 ⑭でご判断いただきたい
- **`floor` / `location` は請求に関係しない**ので、3-4 の制約とは矛盾しない（縛るのは `tenant_code` だけ）
...
テナント設置6件の `受領タイムスタンプ` は作成時刻と**すべて別**（数分後）だった。
...
`調達先` は入庫7件すべて空だった。項目としては残すが、**画面では折りたたむ等、優先度を下げてよい**。
...
| 建物名（右上） | 固定文字列「備後町コイズミビル」（→ 12-2 ⑮。複数物件の予定があれば設定値にする） |
| 階 | テナントの `floor` |
...
- **署名画像が入るので、この PDF は個人情報に近い扱い**。owner には出さない（9章）
...
| 当日登録分の削除 | ○ | ○ | × |
| マスタ（備品・カテゴリ）の編集 | ○ | ○ | × |
| 修理伝票PDF（署名画像を含む）の出力 | ○ | ○ | **×**（下記） |
| 修理データ（請求連携用の表）の出力 | ○ | ○ | ○ |

> **owner にも備品セクションを見せる（2026-08-11 確定。確認事項⑦）。** 当初は「見せない」前提だったので、
> 日報と同じ**閲覧のみ**（書き込みは全てサーバー側で403）で開放する。
>
> ただし **修理伝票PDF だけは owner に出さない**。テナントの**手書き署名画像**が載っており、
> 他社の受領印にあたるため、社内の証跡として扱うのが妥当と考える。請求の根拠として必要な情報は
> 修理データ（8-5-2。テナント・備品・本数・日付）で足りる。**この線引きでよいかご確認いただきたい**（→ 12-2 ⑯）
...
| ⑦ | owner（小泉産業様）に備品セクションを見せるか | **見せる** | 日報と同じ**閲覧のみ**で開放（→ 9章）。ただし署名画像を含む修理伝票PDFは対象外としたい（→ ⑯） |
...
| ⑫ | CSV の列名・件数 | **4本を受領**（カテゴリ7 / 備品22 / テナント52 / 入出庫30件） | 全件を機械検証し、実際の列・使う列・捨てる列を確定（→ 8-1）。**入出庫は抜粋と思われるため再書き出しが必要**（→ ⑰） |

### 12-2. 新たにご判断・ご対応をお願いしたい点（実データの検証で判明。2026-08-11）

受領したファイルを検証した結果、**当初の想定と実データが食い違う点**が見つかった。
いずれも取込スクリプトを書く前に決めておきたい。詳細は 8-4 を参照。

| # | 内容 | こちらの推奨 |
|---|---|---|
| ⑬ | **新規入替なのに請求先コードが入っている行**が1件ある（入出荷ID 1122。備考「1階11-B号室 小泉関連会社入居準備」）。3-4 の「新規入替は請求先を持たない」に反するため、そのままでは取り込めない | **請求先コードを落として取り込む**（階・設置先・備考は残す）。請求実績にも元々載っていないため、落としても請求の履歴は変わらない |
| ⑭ | 「新規入替の入力は備考のみ」とのご回答だったが、**実データでは2件とも設置階・設置先が使われていた**。今後の新規入力でも階・場所を残したいか | 過去分はそのまま取り込む。今後については、**集計できる形で残したいなら階・場所欄を出す**方が良い（備考の自由記述だと後から拾えない）。不要ならご指示どおり備考のみにする |
| ⑮ | 修理伝票の右上「**備後町コイズミビル**」は固定でよいか（他の物件でも使う予定があるか） | 当面は固定文字列。将来を見るなら設定値にしておく（工数はほぼ変わらない） |
| ⑯ | **修理伝票PDF（署名画像入り）を owner に見せるか** | **見せない**。他社の受領印にあたるため社内の証跡として扱い、請求根拠は修理データ（テナント・備品・本数・日付）で足りると考える |
| ⑰ | **入出庫データの全期間の再書き出し**をお願いしたい。受領分は30件・2023〜2026に散在しており、現行画面の年計とも合わないため抜粋と思われる | 全期間でお願いします（⑥で「全件移行可能」とのことでしたので、書き出し条件の見直しだけかと思います） |
