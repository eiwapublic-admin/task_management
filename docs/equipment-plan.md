...
最終更新: 2026-08-11（計画のみ。**確認事項18件すべてに回答をいただき反映済み**＝12章。
...
変更した。新規入替の入力は共通項目＋備考のみで確定。**次のアクションは実装Phase 0〜1の着手**）
...
| 入出庫データ | **830**（2026-08-11 に全期間で再受領。旧30件版は抜粋だったと判明） | 入出荷ID / 入出庫区分 / 入出庫日付 / 入出庫時刻 / 入出庫理由 / 調達先 / **設置先請求先コード** / 設置階 / 設置先 / 備品ID / 入庫数量 / 出庫数量 / 担当者 / 備考 / **受領タイムスタンプ** / 主キー / 作成・修正情報（ほか集計用の `c_*` 列多数） |
...
| 新規入替（4件）に請求先コード・設置階・設置先が入っている行 | **1件あり**（入出荷ID 1122）→ **2026-08-11 に対処方針が確定**（8-4(1)） |
...
#### (1)(2) 新規入替に請求先・設置先が入っている行 ← 【2026-08-11 確定】
...
**【回答・確定】**「除外して投入。またはテーブル制約は無くてよい」（確認事項⑬⑭）を受け、次で確定する。

- **移行**: 新規入替の行は、**理由を問わず一律で請求先コード・設置階・設置先の値を取り込まず、
  備考のみ取り込む**。1122 番もこの規則にそのまま従うので、結果として 1123 番と内容が重なるが、
  どちらも実在した記録として**そのまま2行とも取り込む**（統合や重複排除はしない。件数として
  Σ入出庫と齟齬が出ないようにするため）
- **DBの制約は外す**（4-1）。`equipment_txn_tenant_only_tenant_reason` は**テーブル CHECK 制約としては
...
| Phase | 内容 | 前提 | 目安 |
|---|---|---|---|
| **0. 準備** | 12章の確認事項の回答、CSV 4本の受領、FileMaker 側の到達性方針の決定 | — | **①〜⑫は2026-08-11に回答済み**（到達性は方式A、CSV・帳票の実物も受領・検証済み）。残るは⑬〜⑰の判断と**入出庫データの全期間の再書き出し** |
...
| ⑬⑭ | 新規入替に請求先・設置先が入っている過去データの扱い（実データ検証で1件発見） | **除外して投入。または、入力チェックとしては必要だがテーブル制約は無くてよい** | **移行**: 新規入替の請求先・設置階・設置先は取り込まず備考のみ残す。**設計**: DBのCHECK制約は持たず、`worker/lib/equipment.js` の保存時バリデーション（アプリ層）でのみ強制する形に変更（→ 3-4・4-1・8-4(1)(2)） |
