...
**機能名（タスク／日報／備品）はアプリヘッダのセクション切替が示す。**
だから主要3画面（カンバン・日報一覧・在庫一覧）に画面タイトルは置かない。
サブ画面（アーカイブ・自主検査表・違反車両・残留塩素・処理ログ 等）は現在地が
切替から読み取れないので `title` を渡す。
  ...
  return (
    <div className="ui-page">
      ...
      <div className="ui-container is-narrow">
        <FeatureHeader
          title="画面名"                    {/* 主要3画面では省略する */}
          leading={                         {/* 戻るボタン等（タイトルの前） */}
...
```jsx
<FeatureHeader title="アーカイブ" filters={…} />
...
- [ ] 機能名をタイトルに書いていないか（主要3画面はアプリヘッダの切替が示す）
