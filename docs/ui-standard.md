...
| `--z-header-2` (9) | 2段目・3段目の固定ヘッダー（`.ui-sticky-head` / `-2`）。**必ず `--z-header` より低い値にする**（同値だとDOM順で2段目側が勝ち、ハンバーガーのドロップダウンが下に隠れる不具合になる。2026-08-12に発生・修正済み） |
...
> `.ui-sticky-head` 系の要素はその外側に置く（アーカイブ表の扱いを参照）。
>
