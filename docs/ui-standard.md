...
> **iOS の面倒は `ui.css` が見ている。** アドレスバーで高さが変わる問題（`dvh`）・
> ノッチに隠れる問題（`env(safe-area-inset-top)`）・高さが足りないときのスクロール、
> この3点は `.ui-overlay` / `.ui-modal` で一度だけ対処済み。**画面ごとに書き直さないこと。**
...
| 画面ごとにモーダルの `dvh` / safe-area を書く | 同上 |
