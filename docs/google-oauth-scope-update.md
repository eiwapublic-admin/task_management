...
**場所**: 認証情報 → 該当のOAuthクライアント（`task_management`）→
**「承認済みのリダイレクト URI」の `URI 1`**。行の右端のゴミ箱アイコンで行ごと削除して保存する。
「承認済みの JavaScript 生成元」は今回の作業では触らない。

> **削除してもリフレッシュトークンは無効になりません**。リダイレクトURIは「認可コードの返送先を
> 制限する設定」であり、発行済みトークンの更新には関与しないためです。

> **保存できない場合は残してよい**: これが唯一のリダイレクトURIの場合、Google が
> 「最低1つ必要」として空での保存を拒否することがあります。その場合は**残しておいて問題ありません**。
> このURIを悪用するには `GMAIL_CLIENT_SECRET` が必要で、さらに `eiwa.public@gmail.com` で
> ログインして同意する必要があるため、シークレットが漏れていない限りこの登録単体では何もできません。
> あくまで「使わない設定は残さない」という整理上の作業です。
> なお**次回トークンを再発行する際は、この URI の再登録が必要**になります（削除した場合）。
