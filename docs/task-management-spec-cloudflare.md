...
- **年月移動の右に「本日」ボタン（2026-08-26）**: 押すと表示中の月が今月でなければ今月へ切り替え、`.is-today`（リスト行・カレンダーのマスのどちらにも既に付与済み）まで`scrollIntoView`でスクロールする。月を切り替えた場合は再読み込み（`loading`）が終わるのを待ってからスクロールする。作業記録の検索結果表示中はスクロール対象が無いため表示しない
...
- **祝日名は表示せず日付を赤字にする（2026-09-09。依頼）**: モバイルでの横幅節約のため、
  ...
  休館日のバッジ（「休館日」）は日付が短い文言のため残す。祝日名自体は`aria-label`に含めて
  スクリーンリーダーからは分かるようにしている
