...
- **入力の設計**: 作業記録は「作業記録」見出しバーの**上**にある「＋」ボタンで行が増え（2026-08-26。以前は一覧の下だったが、初見でボタンが見える位置にしたいとの依頼で見出しの上へ移動し、見出し側の件数表示も省略した）、**時刻は空欄が既定値**（後からまとめて入力する運用のため。撮影時刻等と違い現在時刻を入れると実際の作業時刻と食い違う）。
