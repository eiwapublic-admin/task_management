...
- **ハンバーガーメニュー**（上から順に）: ログインユーザー名 / **ダッシュボード** / （区切り線）/ セクションごとの画面（タスクなら カンバン・アーカイブ・設定・従量課金事項・処理ログ・雛形ファイル）/ （区切り線）/ **今すぐ取得** / 通知のオン・オフ / 当システムについて / ログアウト / ビルド時刻。各項目は白背景で統一し、ホバーで淡い青地になる。外側クリック/Esc で閉じる
