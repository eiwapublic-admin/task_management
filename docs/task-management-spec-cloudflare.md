...
- **当日を枠の左端の色で強調（2026-08-07）**: `.report-row.is-today`と同じ配色（緑の左枠3px）を、記録の無い日の行（`.report-day-row`）にも適用し、その日が「記録あり」「未入力」「休館日」のどの状態でも当日だと一目で分かるようにした
...
- **「自主検査入力」ボタンは日報詳細画面へ移動（2026-08-07。当初は一覧画面に置いていたが移設）**: 「該当の日」の自主検査表を直接記録できるショートカットのため、一覧ではなく**日報詳細画面のヘッダ右側**に置く方が自然と判断した。押すと表示中の日付の自主検査表入力モーダル（`InspectionForm`。4-12参照）を開く。開く前にその日の既存記録・休館日状態を取得してからモーダルを表示する
