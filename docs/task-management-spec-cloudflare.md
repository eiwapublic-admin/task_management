...
- **カレンダー型の表示を追加（2026-08-07。PC/iPad専用）**: 従来のリスト型に加え、**横軸が日曜〜土曜の7列のマス目**で1か月を見渡せるカレンダー型を追加し、ツールバーのセグメントボタン（`.report-view-switch`。リスト／カレンダー）で切り替える。月の移動（`‹`/`›`）・対象データ・休館日の登録/解除・行タップでの新規作成といった振る舞いはリスト型と共通で、`dayState()`／`renderIcons()`／`renderClosedDayButton()`を両表示で共用している（`ReportList.jsx`）。マス目の中身もリスト型と原則同じ（日付・「本日」バッジ・作業者・作業記録の抜粋・アイコン3種・休館日）。
  ...
  - **表示分量は未調整**: 1マスに出す作業記録の行数は`CALENDAR_PREVIEW_LINES`（`ReportList.jsx`。暫定3行、超過分は「ほか N 件」）で一元管理している。**実際の画面を見ながら調整する予定のため、この定数を変えるだけで済むようにしてある**
...
- **日報詳細画面のヘッダ再構成（2026-08-07）**: 従来 `justify-content: space-between` で「ホーム／日付＋前日翌日／保存状態」の3要素を並べていたため、画面幅によって日付がホームボタンから離れて見えることがあった。**左グループ（ホーム・前日/翌日・日付）と右グループ（保存状態・自主検査入力・削除）に分け、右側だけ`margin-left:auto`で寄せる**構成に変更（`.report-detail-head-left`/`.report-head-right`）。日付は常にホームボタンのすぐ右に固定される
