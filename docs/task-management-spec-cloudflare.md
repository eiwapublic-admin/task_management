...
- 「Anthropic API 利用状況」カードと「日報の写真ストレージ」カードを別枠に分け、両カードとも見出し行にグレーの塗りつぶしバー（`.usage-panel-title`）を敷いてメリハリを付けている（2026-08-05。当初 `var(--bg-subtle)` を使ったがほぼ白でメリハリが弱いとの指摘を受け、はっきり濃いグレー `#d1d5db` に変更）
