...
> - iOSのズーム対策・モーダルのsafe-area/dvh対応は `ui.css` が一括で見ているので、
>   **画面ごとに書かない**（二重管理になる）
>
...
- タスク詳細モーダル（幅820px・視認性重視で文字は大きめ・モバイルは1カラム）: タイトル+×は固定ヘッダー、最下部は固定フッタ。フッタ左に「ステータス」見出し＋変更ボタン、右に「メール参照」「返信」ボタン
  ...
  - **モバイルのフッタはみ出し対策（2026-07-31）**: iPhoneでフッタ（メール参照・返信）が画面下にはみ出していた。オーバーレイの `padding-bottom` に `env(safe-area-inset-bottom)`（ホームインジケータ領域）を加え、モーダルの `max-height` をオーバーレイの上下パディング＋セーフエリアを差し引いた `calc(100dvh - ...)` にして、sticky なフッタが必ず画面内に入るようにした
