...
- **アーカイブ画面（`/archive`）**: `GET /api/archive` で取得。担当者・情報源での絞り込みと、フリーワード全文検索（タイトル・件名・本文・送信者・宛名・留意事項をサーバー側 ilike で横断。タスクID「T-123」形式での検索にも対応）。一覧には「タスクID」列を表示。行タップで既存のタスク詳細モーダルを開く（編集・ステータス変更も可能）。
