...
- **ハンバーガーメニューは「トップ／業務別メニュー／共通メニュー」の3領域＋区切り線（2026-08-30に再編。193番）**: それまで画面ごとにバラバラに項目を出し分けていた構成を、明確な3領域に整理した。各項目は白背景で統一し、ホバーで淡い青地になる。外側クリック/Esc で閉じる。
  - **トップ領域（iPhoneサイズ＝769px未満のみ表示）**: ログインユーザー名を「〜さん」で表示し、その下に各業務（タスク・日報・備品・違反車両・残留塩素・雛形ファイル）へのショートカット（下記`NAV_ITEMS`と共通）。769px以上ではヘッダー中央の業務メニュー（後述）とユーザー名表示が別途あるため重複を避けて非表示にする（`.app-header-nav`を隠す閾値と同じ`@media (min-width: 769px)`）。旧来「カンバン」表記だった項目はここで「タスク」に改称（`NAV_ITEMS`自体の`label`変更のため、PC幅の業務メニューバーにも同じ表記が反映される）。
  - **業務別メニュー（画面に応じて内容が変化。`AppHeader.jsx`内で`businessMenuItems`として事前計算）**: タスク画面＝**今すぐ取得**／**アーカイブタスク**（旧アーカイブ）／**タスク設定**（旧設定）／**「タスク」の仕組み**（旧・当システムについて。内容がGmail→Claude→カンバンの処理パイプライン説明のためタスク画面専用に配置）。日報画面＝**作業定型文の設定**のみ（owner権限では非表示。旧来あった「日報一覧」への戻りリンクは、トップ領域の業務ショートカットが同じ役割を兼ねるため削除）。備品画面＝在庫一覧／備品マスタ／テナントマスタ（現状通り。備品マスタ・テナントマスタはowner権限で非表示）。雛形ファイル画面・ダッシュボードは業務別メニューなし（該当なし）。
...
- **PC幅の業務メニュー（`NAV_ITEMS`。ショートカットナビ。2026-08-24追加）**: 769px以上でヘッダー中央に「タスク｜日報｜備品｜違反車両｜残留塩素｜雛形ファイル」を横並びで表示し、押すと直接その機能へ移動する（現在地は下線・太字で強調）。狭幅（モバイル）では非表示にし、従来どおりハンバーガーメニュー経由の遷移のみとする。**自主検査表（`/reports/inspections`）は2026-08-30に削除**（下記・4-9・4-12参照）。owner・備品出庫限定ロールにはタスク・雛形ファイルを出さない（`staffOnly`指定。ダッシュボードの導線と同じ扱い）
...
- **その他の導線**: 統計を出すほどではない画面（アーカイブ／処理ログ／備品マスタ／テナントマスタ／
  作業定型文／雛形ファイル／従量課金事項／設定）はカードの下にピル型のボタンで並べる
