...
- タスクカード: **タイトル先頭に情報源アイコン**（上記）/ 送信元（会社・氏名。旧データは From 表示名で代替）/ 担当者アバター / 期限（超過・間近バッジ）/ 受信日時 / **留意事項（remarks、あれば青字で常時表示。2026-07-21に「件名を表示」の開閉トグルから変更）** / **「新着」バッジ（2026-07-22。カード右上。`tasks.updated_at`が24時間以内なら表示。登録時・自動更新・手動編集のいずれでも`updated_at`が更新されるため「取得または更新から24時間以内」の判定にそのまま使える）**
  ...
  - ※スパムボタンは当初カード下部に置いていたが、メイン画面では見せない方針に変更し、タスク詳細（保存ボタンの左）へ移設した（2026-07-31。下記参照）
...
- タスク詳細モーダル（幅820px・視認性重視で文字は大きめ・モバイルは1カラム）: タイトル+×は固定ヘッダー、最下部は固定フッタ。フッタ左に「ステータス」見出し＋変更ボタン、右に「メール参照」「返信」ボタン
  - **1行目に担当者・期限・受信日時・スパム・保存を横並び**表示（`task-detail-toprow`。2026-07-31に受信日時と期限の左右を入れ替え、編集する項目=担当者・期限を左に寄せた）。ステータスはフッタの変更ボタンと重複するため本文側では省略。受信日時は「**2026/07/29 05:58 受信**」の後置き表記にし、要素間の余白と日付入力の幅を詰めて**PC幅で1行に収める**（2026-07-31。従来は「受信日時 ○○」の前置きで、スパム・保存が2行目に折り返していた）。モバイル等で入り切らない場合は自然に折り返す
  - **スパムボタン（2026-07-31。カードから移設）**: 「保存」ボタンの**左**に控えめな「スパム」ボタンを置く。押すとその場に「スパムと判定してすぐにアーカイブへ移動します。〈移動〉〈キャンセル〉」の確認を表示し、「移動」で `PATCH /api/tasks {is_spam:true}` を実行（サーバー側でスパム付与＋完了＋アーカイブを一括処理）→モーダルを閉じ、**画面はメイン（カンバン）に留まる**（2026-07-31。当初はアーカイブ画面へ遷移していたが、続けて他のタスクを処理できるよう変更）。対象カードはカンバンから消える。誤操作でタスクが視界から消えるのを防ぐため確認を挟む。スパム済み・アーカイブ済みのタスクには表示しない（解除はタイトル横のバッジから）
...
- **「自主検査入力」ボタンは日報詳細画面へ移動（2026-08-07。当初は一覧画面に置いていたが移設）**: 「該当の日」の自主検査表を直接記録できるショートカットのため、一覧ではなく**日報詳細画面のヘッダ右側**に置く方が自然と判断した。押すと表示中の日付の自主検査表入力モーダル（`InspectionForm`。4-12参照）を開く。**状態に応じてラベル・見た目を切り替える（2026-08-07）**: その日の記録が未記載なら「自主点検登録」（`.btn-primary`＝青地・白字）、記載済みなら「自主点検修正」（`.btn-plain`＝控えめな見た目）。開く前から出し分けるため、画面表示時（`date`変更時）に先読みしておく（`loadInspectionStatus()`。保存・削除後も再取得してボタン表示を更新する）
- **日報詳細を一覧の上のモーダルに変更（2026-08-07。それまでは別画面）**: `/reports/:date` は**別画面ではなく日報一覧の上に重ねるモーダル**として表示する。ルーティング上は `/reports/:date` も `ReportList` を描画し、`useParams()` の `:date` があるときだけ `<ReportDetail>` をモーダルとして重ねる（`App.jsx`／`ReportList.jsx`）。**URLはそのまま残しているので、違反車両一覧・タスク側の「本日の日報」などからのリンクとブラウザの戻るボタンは従来どおり動く**。閉じると `/reports` へ戻し、一覧を読み直してモーダル内での編集・削除・休館日指定を反映する
  - **ホームボタンと前日/翌日の移動ボタンは廃止**し、ヘッダ右端の「×」だけで閉じる。背景クリック・Escapeでも閉じる。日付は左端、右側は「保存しました／自主点検登録・修正／削除／×」の並び
  ...
  - **入れ子のモーダルの注意点**: 自主点検の入力モーダル（`InspectionForm`。`.inspection-overlay` は `z-index:55`、詳細モーダルは `50`）は詳細モーダルの**オーバーレイの外側**に描画する（内側に置くと自主点検側の背景クリックが詳細側のオーバーレイにも伝播して両方閉じてしまう）。Escapeは `stopPropagation` では同じ `window` の別リスナを止められないため、詳細側は `inspectionOpenRef` を見て自主点検が開いている間は早期returnする（`TaskDetail` の `previewOpenRef` と同じ方式。`multi-env-attachment-preview` スキル参照）
- **日報詳細画面に削除ボタンを追加（2026-08-07）**: ヘッダ右端に日報1件を丸ごと削除するボタン（`ConfirmDeleteButton`。上記`DELETE /api/report`を使用）を追加。自主検査入力ボタンとは`.report-head-delete`の`margin-left`で少し間隔を空けている（誤操作防止）
...
- **画面（`/reports/chlorine`）**: 現行アプリのTOP・一覧・帳票出力を1画面にまとめた。上から
  「測定を記録」ボタン → 絞り込み・帳票出力（対象年・帳票の出力対象施設・PDF出力アイコンを1行） → 測定日ごとの一覧。
  一覧自体は施設で絞り込まない（両施設まとめて見たいとの要望。2026-08-10）。帳票の出力ボタンは見出しラベルを省略し
  `IconDownload` のみのアイコンボタン（`.icon-btn-download`）にして、年・施設・出力ボタンがiPhone幅でも1行に収まる
  ようにした。一覧は日付見出しの下に施設名・測定時刻・測定値・測定場所・検査者を並べ、**行タップで入力モーダル**を開く
...
- **入力モーダル（`ChlorineForm.jsx`）**: 現行アプリの入力画面の並びに合わせ、測定施設 → 測定場所・検査者 →
  ...
  **測定場所は、新規登録時にその施設で最後に記録した場所名を自動取得して既定表示する**（`GET /api/report/chlorine?building=`
  ...
  既存レコードの編集時は行わない）。**残留塩素濃度**は自由入力の数値欄に加え、よく使う値（0.05/0.10/0.20/0.30/0.40/0.50）
  を選べる小さなドロップダウンを横に並べている（選択すると数値欄に反映され、ドロップダウン自体は選択直後に空へ戻る。
  作業記録の「定型文から追加」と同じ操作パターン）
...
- **日報詳細からの入力（2026-08-10）**: 自主点検ボタンと同じ配置・考え方で、日報詳細のヘッダーに「残留塩素登録」
