...
>
> 1. **アプリヘッダ**（`AppHeader`。グリーンの帯）… 左＝アプリアイコン＋セクション切替
>    （タスク／日報／備品）、右＝ログインユーザー名（769px以上）＋ハンバーガーメニュー
> 2. **機能ヘッダ**（`FeatureHeader`）… 左＝年月・日付選択／表示切替／絞り込み（検索を含む）、
>    右＝その機能のボタン群。狭幅では右側が2行目の右端に回る
> 3. **本体**… 一覧。縦スクロールしても 1・2 は固定表示のまま
>
> 機能名（タスク／日報／備品）はアプリヘッダのセクション切替が示すため、主要3画面
> （カンバン・日報一覧・在庫一覧）に画面タイトルは置かない。サブ画面（アーカイブ・
> 自主検査表・違反車両・残留塩素・処理ログ 等）は現在地が切替から読み取れないので
> タイトルを置く。一覧の「集計見出し」（備品なら備品名、残留塩素なら測定日）は
> 本文より一段大きく・太くして、どこで区切れているかを一目で分かるようにする。

**共通ヘッダー（`src/components/AppHeader.jsx`。2026-07-17に共通化、2026-08-12に再構成）**: 全画面で同一のヘッダーコンポーネントを使う。落ち着いたグリーン（#33604d→#24483a の浅い縦グラデーション）。左に栄和ロゴ＋セクション切替、右に**ログインユーザー名**（769px以上。狭幅ではドロップダウンの先頭に出す）と**常時3本線（ハンバーガー）メニュー**（画面幅によらず）。各画面固有の「×で閉じる」ボタンは無く、ハンバーガーメニューから自由に行き来する。
...
- **狭幅の2行固定レイアウトを撤去（2026-08-12）**: アプリ名が無くなり狭幅でも1行に収まるようになったため、2026-08-07 に入れた `grid-template-areas` での行割り当てと、モバイル専用に複製していたセクション切替（`.app-switch-mobile`）を削除した。ヘッダーは常に「ロゴ＋切替 ／ ハンバーガー」の1行
