...
- **タスク・メール実績からの自動作成**: 一覧画面の「連絡帳を自動作成」ボタン（`POST /api/contacts/sync`）から、スタッフが任意のタイミングで実行する。Gmail取得のたびにリアルタイムで作る方式ではない。`tasks`テーブルのうちスパム除外・`sender_email`が入っているものをメールアドレス単位（大小文字を無視）で名寄せし、まだ連絡帳に無いアドレスだけ新規作成する。**既存の連絡先は上書きしない**（手動編集した内容を後から壊さないため）。会社名の初期値は`sender_display`→`contact`→メールアドレスの順でフォールバックする
...
- **未実施**: 依頼元による実機確認。このセッションにはSupabase/wrangler devの資格情報が無く、ビルド成功・lintクリーンまでの確認に留まっている
