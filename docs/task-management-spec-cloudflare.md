...
- **日報詳細を一覧の上のモーダルに変更（2026-08-07。それまでは別画面）**: `/reports/:date` は**別画面ではなく日報一覧の上に重ねるモーダル**として表示する。ルーティング上は `/reports/:date` も `ReportList` を描画し、`useParams()` の `:date` があるときだけ `<ReportDetail>` をモーダルとして重ねる（`App.jsx`／`ReportList.jsx`）。**URLはそのまま残しているので、違反車両一覧・タスク側の「本日の日報」などからのリンクとブラウザの戻るボタンは従来どおり動く**。閉じると `/reports` へ戻し、一覧を読み直してモーダル内での編集・削除・休館日指定を反映する
  - **ホームボタンと前日/翌日の移動ボタンは廃止**し、ヘッダ右端の「×」だけで閉じる。背景クリック・Escapeでも閉じる。日付は左端、右側は「保存しました／削除／×」の並び（**自主点検登録・修正／残留塩素登録・修正のショートカットボタンは2026-08-13に廃止**。下記参照）
...
- **自主点検・残留塩素の入力ショートカットを廃止し、削除・×を日付と同じ行に固定（2026-08-13）**: 上記「自主点検登録/修正」「残留塩素登録/修正」の2ボタンをヘッダから削除した（記録は各専用画面（自主検査表・残留塩素等検査）の行タップ・「＋」から行う運用に一本化）。これに伴い `ReportDetail.jsx` から自主点検・残留塩素それぞれの状態取得（`loadInspectionStatus`/`loadChlorineStatus`）・開閉ハンドラ・`InspectionForm`/`ChlorineForm` の描画（詳細モーダルのオーバーレイ外に出していたもの）を丸ごと削除した。残ったのは「保存しました」表示・削除（ゴミ箱）・×のみで、狭い画面でも折り返さず必ず日付と同じ行（トップ）に収まるようになったため、`.report-detail-head`/`.report-head-right` の `flex-wrap: wrap`（4要素が並ぶ狭幅での折り返し対策として2026-08-10に追加していたもの）は不要になり削除した
