...
10. `last_fetch_at` を現在時刻に更新。**ここまで完走したときだけ前進する**のが重要で、途中で落ちた実行で前進させるとそのメールを永久に取りこぼす（2026-07-27の事例）。逆に前進しないまま実行だけが繰り返される状態は手順2の凍結検知で拾う
...
- **ハンバーガーメニューは「トップ／業務別メニュー／共通メニュー」の3領域＋区切り線（2026-08-30に再編。193番）**: それまで画面ごとにバラバラに項目を出し分けていた構成を、明確な3領域に整理した。各項目は白背景で統一し、ホバーで淡い青地になる。外側クリック/Esc で閉じる。
  - **トップ領域（iPhoneサイズ＝769px未満のみ表示）**: 各業務（タスク・日報・備品・違反車両・残留塩素・雛形ファイル・連絡帳）へのショートカット（下記`NAV_ITEMS`と共通）。769px以上ではヘッダー中央の業務メニュー（後述）が別途あるため重複を避けて非表示にする（`.app-header-nav`を隠す閾値と同じ`@media (min-width: 769px)`）。旧来「カンバン」表記だった項目はここで「タスク」に改称（`NAV_ITEMS`自体の`label`変更のため、PC幅の業務メニューバーにも同じ表記が反映される）。**ログインユーザー名の表示と「ダッシュボード」リンクは2026-09-02に削除**（ユーザー名はPC幅の`.app-header-user`のみに残す）。**ダッシュボード（`/portal`）画面自体では、この領域を区切り線ごと丸ごと非表示にする**（同じ内容をタイルで既に見せているため。204番）
  - **業務別メニュー（画面に応じて内容が変化。`AppHeader.jsx`内で`businessMenuItems`として事前計算）**: タスク画面＝**今すぐ取得**／**アーカイブタスク**（旧アーカイブ）／**タスク設定**（旧設定）／**「タスク」の仕組み**（旧・当システムについて。内容がGmail→Claude→カンバンの処理パイプライン説明のためタスク画面専用に配置）。日報画面＝**自主検査表を出力**（モバイル幅限定。`.app-menu-mobile-only`。日報一覧のヘッダーにあった自主検査表PDF出力ボタンをモバイル幅ではここへ移し、ヘッダー行を1行に収めた。PC幅では引き続きヘッダー側のボタンから実行できる。203番）／**作業定型文の設定**（owner権限では非表示。旧来あった「日報一覧」への戻りリンクは、トップ領域の業務ショートカットが同じ役割を兼ねるため削除）。備品画面＝在庫一覧／備品マスタ／テナントマスタ（現状通り。備品マスタ・テナントマスタはowner権限で非表示）。**連絡帳画面＝連絡帳を自動作成のみ（モバイル幅限定。PC幅では引き続きヘッダー側のボタンから実行できる。203番）**。雛形ファイル画面・ダッシュボードは業務別メニューなし（該当なし）。
  - **共通メニュー（全画面共通）**: 連絡帳・リマインダー（2026-09-07。4-17）・従量課金事項・処理ログ（いずれもowner権限では非表示）・通知のオン/オフ・ログアウト・ビルド時刻。
  - ダッシュボード下部の導線（`Portal.jsx`の`linkGroups`）も、名称変更した項目（設定→タスク設定）を
    あわせて改称済み。**アーカイブ→アーカイブタスクの改称は2026-09-05にダッシュボード側のリンク自体を
    削除したため現存しない**（アーカイブへの導線はタスク画面側の一本化ボタンのみ。上記4-3参照）。
- **PC幅の業務メニュー（`NAV_ITEMS`。ショートカットナビ。2026-08-24追加）**: 769px以上でヘッダー中央に「タスク｜日報｜備品｜違反車両｜残留塩素｜雛形ファイル｜連絡帳」を横並びで表示し、押すと直接その機能へ移動する（現在地は下線・太字で強調）。狭幅（モバイル）では非表示にし、従来どおりハンバーガーメニュー経由の遷移のみとする。**自主検査表（`/reports/inspections`）は2026-08-30に削除**（下記・4-9・4-12参照）。owner・備品出庫限定ロールにはタスク・雛形ファイル・連絡帳を出さない（`staffOnly`指定。ダッシュボードの導線と同じ扱い）
...
- **スクロールバーの見た目（2026-07-21。2026-08-26に構造変更あり）**: Windows/macOSのデスクトップブラウザ既定のオーバーレイ型スクロールバーは、ページのスタッキング（sticky固定のヘッダー等）とは無関係にOS/ブラウザのUI層で最前面に描画されるため、sticky固定ヘッダーに食い込んで見えることがある。常時表示の細いスクロールバーを自前描画するCSS（`scrollbar-gutter: stable` + `::-webkit-scrollbar` 系 + Firefox の `scrollbar-width`/`scrollbar-color`）で、通常のスタッキング順に従わせることで解消している。**ただしSafariでは未解消**（Safariはメインページのスクロールを`::-webkit-scrollbar`等ではなくOS統合の表示（NSScroller）で描画する傾向が強く、ページ側CSSでの制御が効きにくい。業務に支障が無いためユーザー了承のもと対応保留）。**当初は`html`要素にこのCSSを付けていたが、iOSステータスバー霞対策のapp-shell化（170番。下記PWA節）で実際にスクロールする要素が`html`から各画面の`.app-scroll`（`.ui-container`と同一要素）へ変わり、`html`側の`scrollbar-gutter`が窓の真の右端に無駄な余白を確保し続ける一方、実スクロールバーは中央寄せされた`.app-scroll`自身の右端（＝窓の内側）に出る二重のズレが発生した。177番で、これらのCSSを`.app-scroll`／`.settings-container`（設定画面専用）へ移設し、あわせて`.ui-container`の中央寄せ方式を`max-width`+`margin-inline: auto`から`width: 100%`+`padding-inline: max(通常パディング, (100% - 目標幅) / 2)`に変更（要素自身は常に窓いっぱいの幅を保ち、パディングだけで見た目を中央に寄せる）することで解消した。実機で「完璧に直りました」と確認済み**
...
- **起動時の既定表示（2026-09-02に変更。204番）**: PC/iPad幅は従来どおりカンバン（`/`）のまま。
  **モバイル幅（`(max-width: 768px)`）だけは、アプリ起動直後の最初の`/`遷移をこの画面（`/portal`）へ
  振り替える**（`src/App.jsx`の`DefaultRoute`。モジュールスコープのフラグで「起動後まだ振り替えて
  いない最初の1回」だけに限定しており、ヘッダー等から明示的に「タスク」を選んで`/`へ来た場合は
  モバイルでもカンバンをそのまま表示する）。PWAの`manifest.webmanifest`の`start_url`も`/`のため、
  ホーム画面から起動した場合も同じ判定が働く
- **見出し（日付・挨拶）の見た目を入れ替え（2026-09-02。204番）**: 依頼により、日付表示
  （`.portal-hero-date`）を挨拶文と同じ大きさ・太さ・濃い文字色に、挨拶文（`.portal-hero-title`）を
  日付と同じ小さく控えめな見た目に変更した（並び順＝日付が上、は変更していない）
- **アーカイブボタンをタスク画面側へ一本化（2026-09-05。205番）**: 従来はダッシュボードの
  「その他」導線とタスク画面（カンバン）ヘッダーの両方にアーカイブへの入口があったが、
  ダッシュボード側は削除しタスク画面側のボタンだけにした。あわせてそのボタン自体を
  ひとまわり大きく（`.icon-btn-archive`。既定サイズ+8px）、常時色付きの塗り＋枠線
  （`--color-primary-soft`背景・`--color-primary`枠線）で目立たせるよう変更した
  （「もう少し大きく目立つように」との依頼）
- **カードと統計値**: タスク＝未処理件数（補足に対応中・期限超過）／日報＝当月の記録日数
  （補足に本日分の記録有無）／備品＝在庫わずかの品目数（補足に登録品目数）／
  違反車両＝当月の記録件数（補足に累計）／残留塩素＝当年の測定件数（補足に最終測定日）。
  対応が要る数字（期限超過あり・在庫わずか・本日の日報が未入力）は赤で表示する
  - **自主検査カードを削除（2026-08-30）**: 自主検査表画面（`/reports/inspections`）への導線を
    ダッシュボード・業務メニュー・ハンバーガーメニューのいずれからも外すことになったため、
    このカードも合わせて削除した（4-9・4-12参照）
  - **タイル右上のショートカットボタン（2026-09-02追加。204番）**: カード本体のクリック（一覧へ遷移）
    とは別に、角に重ねた小さなボタン（`.portal-card-quick`）を置き、押すとその画面へ遷移した上で
    特定の操作を直接開く。日報＝「本日」（`/reports/:今日の日付`へ遷移し本日の詳細をそのまま表示）、
    違反車両・残留塩素＝「＋」（`navigate(path, { state: { openNew: true } })`で遷移し、遷移先が
    `location.state.openNew`を検知して新規入力を自動で開く。owner等の書き込み不可ロールには
    出さない）。ボタン以外の領域を押せば従来どおりカード全体のクリックとして一覧へ遷移する
- **その他の導線（2026-09-05に3グループへ再編。205番）**: 統計を出すほどではない画面へのリンクは、
  カードの下に**「その他機能」「マスタ」「管理」**の3つの見出し付きグループでピル型のボタンを並べる。
  **アーカイブへのリンクはここから削除**し、タスク画面（カンバン）側の導線に一本化した（下記参照）
  - **その他機能**（記録の参照）: 雛形ファイル／連絡帳
  - **マスタ**（各種マスタの管理）: 日報用作業定型文（旧「作業定型文」）／備品マスタ／
    備品用テナントマスタ（旧「テナントマスタ」）／ビルメン作業マスタ（旧「作業マスタ」。
    owner（小泉産業様）も閲覧できる。`docs/bilmen-plan.md` 10章）
  - **管理**（設定・運用管理）: 処理ログ／タスク設定／従量課金事項／リマインダー
    （2026-09-07追加。4-17参照）
  - グループはリンクが1件も無ければ（全件 `staffOnly` かつ owner）非表示にする
  - **グループの配列を転置（2026-09-07。依頼）**: 当初は3グループを縦に並べ、各グループ内の
    リンクをピル状に横並び・折り返しで配置していた。上のメインカード（`.portal-grid`）と
    「列」の位置が揃わず見づらいとの指摘を受け、`.portal-link-groups`をCSS Grid
    （`.portal-grid`と同じ`repeat(auto-fit, minmax(...))`の考え方）にしてグループ名を
    左から順に横並びにし、各グループ内のリンクボタンはグループ名の下に縦方向に並べる
    （`.portal-links { flex-direction: column }`）構成へ転置した。600px以下では
    `.portal-link-groups`を1カラムに畳む（各グループ内は引き続き縦並び）
...
- タスク詳細モーダル（幅820px・視認性重視で文字は大きめ・モバイルは1カラム）: タイトル+×は固定ヘッダー、最下部は固定フッタ。フッタは右寄せで「返信」「日報に追加」「保存」ボタン（2026-08-18。下記参照）
  ...
  - **「送信者」の上に「顧客」の独立した行**を表示（2026-09-01。以前は送信者の右隣に`contact`を表示していた）: `contact_company`/`contact_person`（先方＝顧客を会社名・氏名に構造化した列。4-16の連絡帳と共通）を使い、「顧客　{会社名}　担当者：{顧客担当者}」の形式で表示する。どちらも無い場合は`contact`の結合文字列にフォールバックする
  ...
  - **メール参照**: Gmail のウェブ画面で該当メールを開く（`https://mail.google.com/mail/?authuser=<共有アドレス>#all/<gmail_message_id>`。パスに `/u/<アドレス>` を埋め込む形式は 404 になることがあるため authuser クエリを使う。共有アカウントへのログインが必要）。ボタンは**フッタ**に配置（2026-09-01、フッタへ再移設。2026-08-18に本文プレビュー欄の右肩へ移設していたが依頼元の指示で元に戻した）
...
**処理ログ画面（/logs）**（`AppHeader`＋`FeatureHeader`。機能名「処理ログ」はアプリヘッダのタイトルが示すので画面側にタイトルは置かない。「×」で閉じるボタンは無く、ハンバーガーメニューから他画面へ遷移）:
- 直近200件を表形式で表示: 日時 / 種別（メール取得・ステータス変更・**バックアップ**）/ 実行者 / 内容
- 実行者は担当者の表示名（手動操作）または「システム（自動）」（Cron 実行・返信自動検知・日次バックアップ）
- **表示幅は画面いっぱい**（2026-07-18: `.logs-container` の `max-width: 1100px` を撤廃。広い画面で余白を残したまま列が折り返される問題を解消）
- **種別フィルタ・日ごとのグルーピング（2026-08-17）**: 機能ヘッダーの `filters` 側に種別（すべて／メール取得／ステータス変更／バックアップ）を絞り込むフィルタチップを設置。一覧はJST日付ごとにグルーピングし、日の見出し行をクリックで開閉できる（残留塩素等検査画面の年月グループと同じ「既定値＋ユーザー上書き」方式。既定は直近7日を開き、それより古い日は閉じる）。日の見出し行自体が境界を示すため、従来の日の区切り線（太線）は廃止した
  - **見出し行の視認性向上（2026-08-29）**: 日の区切りが見つけにくいとの指摘を受け、見出し行の背景を`var(--bg-subtle)`（ほぼ白）から濃いグレー（`var(--text-faint)`。従量課金画面の見出しバーと同じ色）に変更し、日付ラベルの文字も一段大きくした（`.logs-day-label`を`--fs-lg`に）
- **再読み込みボタン（2026-08-27）**: 機能ヘッダーの `actions` 側（右上）に円形の矢印アイコンのボタンを設置。押すと一覧を再取得する（既存の一覧は残したまま更新し、読み込み中はアイコンが回転して進行中と分かるようにする）
...
- **「AI利用の1日あたり上限」カード（2026-09-04）**: サーキットブレーカーの上限値（`settings.daily_api_cost_limit_usd`）の入力欄。当初はタスク設定に置いたが、金額の設定は課金実績と同じ画面で見られる方が分かりやすいため依頼により移設した。入力値が `LOW_LIMIT_WARN_USD`（0.10ドル）を下回っている間は赤い警告を出す（平常の利用実績0.06〜0.08ドル/日を下回る上限は、歯止めそのものが業務を止める側に倒れるため）
- **「運用上の留意事項・設定要領」カード（2026-09-05。依頼）**: 2026-09-03の課金事故（9章）で得た知見のうち、**運用する人が知っておく必要があること**だけを画面に出す。設計の詳細・経緯はドキュメント側に置き、画面は次の4節に絞る: ①平常時の目安（0.06〜0.08ドル/日・0.2ドル/日超は異常・確定額はAnthropicの請求が正）、②AIが止まったときの見分け方（クレジット不足／1日の上限到達／処理ログの赤いエラーの3つを表で対比し、それぞれの対処を書く）、③設定の要領（上限は0.10ドルを下回らない・Anthropic側の自動リチャージはオフ・「今すぐ取得」を連打しない）、④処理ログの「次回へ繰り越し」と外部リクエストの8割警告の読み方（まず進行中タスクを整理する）。表は `.usage-guide-table-wrap` で表だけ横スクロールさせ、ページ本体は横に溢れさせない
...
- **権限ロール（`users.role`）**: `staff`（既定・従来どおり全機能）／`owner`（日報・備品・自主検査・違反車両・残留塩素の閲覧のみで書き込み不可。2026-08-25追加）／`admin`（将来の分離用。当面 staff と同等）／`equipment_out_staff`（2026-08-25追加。上記5画面の閲覧に加え、備品画面での「出庫」新規登録・当日入力分の「出庫」修正のみ書き込み可。過去データの誤修正を避けるため入庫・当日以外の出庫の修正／削除は不可。詳細は`equipment-plan.md`9章）。
  既存ユーザーは全員 `staff` になるため挙動は変わらない。**ロールは JWT に埋めた値ではなく `verifyRequestAuth` が毎回DBから引く**ため、
  権限を下げた際に有効期限（30日）を待たず即反映される。`owner`・`equipment_out_staff` にはフロントでも書き込みUIを出さないが、
  **サーバー側でも各APIの書き込みを403で拒否**し、UIの無効化だけに依存しない（`worker/lib/http.js` の `canWrite`／`isOwner`／`isEquipmentOutStaff`／非公開の`isReadOnlyRole`＝両ロールをまとめた判定。フロントは`src/lib/auth.js`の`isLimitedRole(user)`で両ロールを同列に「閲覧専用」として扱う）。
  備品の出庫登録・修正・署名登録の3エンドポイントだけは例外で、`equipment_out_staff`について`canWrite`とは別に「対象が`kind='out'`かつJST暦日で当日」を条件に個別許可する
- **画面**: 日報一覧（`/reports`）／日報詳細（`/reports/:date`。写真・違反車両の登録を含む）／自主検査表（`/reports/inspections`）／違反車両一覧（`/reports/parking`）／残留塩素等検査（`/reports/chlorine`）／定型文の設定（`/reports/templates`）。
  タスク管理側の各ルートは `RequireStaff` で包み、`owner`・`equipment_out_staff` が来たら `/reports` へ送る
- **入力の設計**: 作業記録は「作業記録」見出しバーの**上**にある「＋」ボタンで行が増え（2026-08-26。以前は一覧の下だったが、初見でボタンが見える位置にしたいとの依頼で見出しの上へ移動し、見出し側の件数表示も省略した）、**時刻は現在時刻を5分単位で切り捨てた値が既定値**（例: 09:07 → 09:05。`nowHHMMFloor5()`。2026-09-09に依頼で変更。当初は「後からまとめて入力する運用」を想定して空欄にしていたが〈2026-08-04〉、実際は作業のたびにその場で追加するため既定値がある方が早い。切り上げにしないのは、まだ終わっていない時刻を書かないため。違っていればその場で打ち直せる）。
  **追加すると、その行の時刻欄へ画面がスクロールして文字入力状態になる**（2026-09-09。
  依頼。既定値を直すためすぐ入力を始められるように）。`focus()`だけではブラウザ任せの
  自動スクロールになり実機で効かないことがあるため、`focus({preventScroll:true})`で
  自動スクロールを止めたうえで`scrollIntoView({behavior:'smooth', block:'center'})`を
  明示的に呼ぶ（`focusEntryIdRef`で追加した明細の`id`を一時保持し、`entries`更新後の
  `useEffect`で`data-entry-id`属性から対象行を探す）。
...
- **一覧行の作業者表記**: 「午前：○○　午後：○○」から「○○ | ○○」の文字表記に簡略化（2026-08-05）→ **2026-08-26にカレンダー表示と同じ丸いアバターマーク（`renderCalendarWorkers`。カンバンのタスクカードと同色）に統一**。リスト表示・作業記録の検索結果のどちらも対象
...
- **未入力の日に「休館日」ボタンを表示（2026-08-07。2026-09-02に未来日も対象化。203番）**: 日報の無い日の行の右端に「休館日」ボタンを置き、押すとその日を休館日にする（行がグレー背景になり「休館日を解除」ボタンに変わる）。**当初は未来日を除いていたが、休館日が事前に分かっている場合（祝日等）に先回りで登録できるよう、未来日も対象にした**（`renderClosedDayButton()`の`isFuture`判定を撤去。自主検査表画面（`Inspections.jsx`）側の行クリック制限も同様に緩和した）。休館日は自主検査表専用ではなく**プロジェクト共通の情報**（`closed_days`テーブル。下記4-12参照）で、日報一覧・自主検査表のどちらから登録・解除しても両画面に反映される
- **日曜・祝日は自主検査表の休館日スイッチを既定でオンにする（2026-09-02。203番）**: `InspectionForm`（4-12参照）の「休館日」トグルは、既存の点検記録・休館日登録のどちらも無い（＝まだ何も決まっていない）日曜・祝日を開いたときだけ、既定でオンの状態にする（`weekdayInfo(date, holidays).isRed`を`isSundayOrHoliday`として渡す。`src/lib/reports.js`の`weekdayInfo()`に`isRed`を追加）。実際に営業した場合はスイッチをオフに戻せば通常どおり記録できる。日報一覧・自主検査表・日報詳細のいずれから開いても同じ既定値になる
- **年月移動の右に「本日」ボタン（2026-08-26）**: 押すと表示中の月が今月でなければ今月へ切り替え、`.is-today`（リスト行・カレンダーのマスのどちらにも既に付与済み）まで`scrollIntoView`でスクロールする。月を切り替えた場合は再読み込み（`loading`）が終わるのを待ってからスクロールする。作業記録の検索結果表示中はスクロール対象が無いため表示しない。**スクロールに加えて本日の詳細も開く**（2026-09-09。依頼。以前はスクロールするだけで、結局そこから本日の行をもう一度タップする必要があった）。既存の行クリックと同じ判定で、日報が既にあればそれを開き、無ければ（作成できる立場かつ休館日でなければ）新規作成して開く。owner・休館日で未作成の場合は行クリックでも開けないため、ここでも何もしない
...
- **会議室予約表のPDF出力（2026-08-29）**: カレンダー型表示のときだけ、機能ヘッダ右上（`actions`側）に「↓ 会議室予約表」ボタン（`.icon-btn-download-meeting`）を表示する。押すと表示中の月の「備後町コイズミビル５階　会議室利用申込書」を紙の様式に寄せたA4横PDFで作成する（18列の表が横に長いため縦ではなく横向き）（`useMeetingRoomPdfExport.jsx`／`MeetingRoomSheet.jsx`。自主検査表PDF出力と同じ作り方で、画面外にシートを描画してhtml2canvasで撮る）。日付・曜日の2列＋9:00〜18:00を30分刻みにした18列の表で、記入例の行（10:00〜12:30の予約例）を先頭に固定で入れる。**土曜＝薄い青、日曜・祝日・休館日＝薄いピンクの背景**を付け、記入時にひと目で分かるようにする（祝日は`GET /api/report/holidays`、休館日は`closed_days`をそれぞれ参照）。半月ごとに1ページとし、月の日数をちょうど半分に分ける（`halfMonthDayRanges()`。自主検査表の16列固定の分け方とは異なり、行数の制約が無いため単純な半分割で済む）。**記入例の予約時間帯（10:00〜12:30）は下線ではなく実際の両矢印**（線＋左右の三角形。`.mr-example-arrow`）で示す。**毎時00と30の間の縦罫線は毎時の境界線より細い点線**にして区別している（`.mr-row-minutes`/日の行のセルに`nth-child`で個別指定。2026-08-30）
...
- **日報一覧の「自主検査表」ボタンを「自主検査PDF」に変更（2026-08-07）、後に「自主検査」へキャプション短縮（2026-08-08）**: 従来は自主検査表の一覧画面（`/reports/inspections`）へ遷移するボタンだったが、**その場で表示中の月のPDFを作成してアプリ内プレビュー表示する**ボタンに変更した（アイコンも自主検査表画面のPDFボタンと同じ`IconDownload`に統一）。これにより一覧画面（`/reports/inspections`）への通常の遷移手段は無くなったが、**画面自体は削除せず残している**（様子見。URL直打ちや将来の用途のため）。PDF生成ロジック（html2canvas＋jsPDFでのシート撮影・アプリ内プレビュー用のアップロード・共有ボタン）は元々`Inspections.jsx`にしかなかったものを`useInspectionPdfExport(month, building)`フック（`src/hooks/useInspectionPdfExport.jsx`）へ切り出し、自主検査表画面・日報一覧（リスト型・カレンダー型とも同じボタン）の両方から使う。フックは呼び出し側が既に読み込んでいるデータに依存せず、ボタンが押された時点で対象月の実施記録・休館日・祝日を自分で取得するため、PDFを使わない画面（日報一覧の通常表示）を重くしない。**実機（iPhone）でリスト型・カレンダー型どちらからもPDFプレビューが開くことを確認済み（2026-08-08）**
  - **自主検査表画面の導線削除に伴う再移設（2026-08-30）**: 業務メニュー・ダッシュボード・ハンバーガーメニューから自主検査表画面（`/reports/inspections`）への導線をすべて削除したことに伴い（4-12参照）、そちらにあった「印刷用」PDFボタン（`.icon-btn-download-print`）を日報一覧の機能ヘッダ（`actions`側。会議室予約表ボタンと並び、キャプションは「自主検査」）へ一本化した。フック自体は元々共用の`useInspectionPdfExport(month, building)`のため実装の追加は無く、呼び出し元を`Inspections.jsx`から`ReportList.jsx`へ移しただけ。表示形式（リスト／カレンダー）に関わらず常に表示する（会議室予約表ボタンはカレンダー型のときだけ）
  - **モバイル幅ではハンバーガーメニューへ再移設（2026-09-02。203番）**: 機能ヘッダの他の絞り込み・並べ替えと合わせるとモバイル幅で1行に収まらないため、`.icon-btn-download-print`自体はモバイル幅（`max-width: 768px`）で非表示にし、代わりにハンバーガーメニューの業務別メニュー（「作業定型文の設定」の上）に同じ操作（「自主検査表を出力」）を追加した（`AppHeader.jsx`の`onInspectionDownload`/`inspectionDownloadBusy`プロップ。`ReportList.jsx`だけが渡すため他の`/reports`配下の画面では出ない）。PC幅（769px以上）は従来どおりヘッダー側のラベル付きボタンのまま
...
- **画面自体は一旦ペンディング（2026-08-30）**: 業務メニュー（PC幅のショートカットナビ）・ダッシュボード・
  ...
  日報一覧画面（4-10参照）のヘッダへ移設**して引き続き使えるようにしてある
  （`useInspectionPdfExport`フックをそのまま流用。`ReportList.jsx`）。今後この画面自体を
  使う場面が無いと判断されれば、ルート・実装ごと廃止する見込み
...
- **モバイルは撮影・ライブラリ選択の両方を用意（2026-09-02。206・207番）**: 従来はモバイル幅で「撮影して追加」
  （カメラのみ）しか選べず、既に端末にある写真を選べなかった。新規登録の追加枠（`ReportParkingViolations.jsx`）・
  2枚目までの追加枠（同ファイルの`ParkingCard`）・違反車両一覧の明細クリックで開く詳細モーダル
  （`ParkingViolationDetail.jsx`）の3か所すべてで、モバイル幅は「撮影」「ライブラリから選ぶ」の2つの
  アイコンボタン（`.icon-btn-camera`/`.icon-btn-folder`。残留塩素の測定写真ボタン・4-14と同じ見た目）を
  左右に並べて表示するよう統一した（PC幅は従来どおり「ファイルから選ぶ」の1ボタンのまま）
...
- **リスト選択（2026-08-05。2026-08-19に地域・メーカー・車種・所有会社を`<datalist>`→`Combobox.jsx`へ変更）**:
  地域・メーカー・車種・所有会社（テナント名）は、過去の全レコードから重複を除いた値を`Combobox.jsx`の候補として
  選べる（自由入力も可能）。`<input list>+<datalist>`はiOS/iPadOS Safariで候補が正しく出ない不具合があるため
  （備品管理と同じ理由。HANDOFF 118番）、備品側と同じ自前の候補リスト部品に統一した。**ナンバーだけは今回の
  対象外で`<datalist>`のまま**（同じ不具合が残っている可能性がある）。候補は`GET /api/report/parking`
  （`report_id`省略・全期間）を1回取得して生成する。**所有会社・訪問先は必須項目**（2026-08-19）: 空欄のときは
  オレンジ枠で入力を促し、明示的な保存ボタンを持つ詳細モーダル（`ParkingViolationDetail.jsx`）は保存時に空欄
  なら弾く（不明な場合は「（不明）」と入力する運用）。自動保存の日報詳細カード（`ReportParkingViolations.jsx`）
  は保存を止める操作自体が無いため、視覚的な必須表示のみ
- **一覧画面（`/reports/parking`）**: 日報入力とは独立させ、`report_id`を指定しない`GET /api/report/parking`で
  ...
  **「＋」も同日中に`filters`側へ移した（2026-08-13。上記の直後）**: `actions`は狭幅（≤768px）で機能ヘッダの
  ...
  （記録元の日報を開けば確認できる）
- **明細カードの行構成（2026-08-19導入・2026-09-07修正）**: `.parking-list-row`は3行構成。1行目＝日付・ナンバー
  （＋同一ナンバーの累計回数が2回以上のときだけ`累計N回`バッジ）、2行目＝違反事項タグ（`.parking-list-tags`）、
  3行目＝所有会社（青太字）→メーカー→車種の順（`.parking-list-sub`）。当初は分類タグを1行目の右側に
  `justify-content: space-between`で寄せる構成だったが、1行目の内容量（累計バッジの有無）によって
  タグが1行目に収まったり2行目へ折り返したりバラつく不具合があったため、`.parking-list-tags`に
  `flex-basis: 100%`を指定して常に2行目へ固定した（依頼による修正。`src/pages/Dashboard.css`）
...
- **モバイル幅でのレイアウト崩れ修正（2026-08-05）**: `.report-card`にはヘッダー帯の演出用に`overflow: hidden`が
  ...
  `.parking-card-photos`をflexからCSS Grid（`repeat(auto-fill, minmax(120px, 1fr))`。当時の作業写真一覧の
  モバイル時と同じ値だったが、2026-08-26に`.photo-grid`側だけ`1fr`へ変更したため現在は値が異なる）に
  変更し、`.parking-card`・`.parking-card-photo-wrap`に`min-width: 0`を追加して解消した
...
- **ダッシュボード（月別台数推移・テナント別/車別ランキング。2026-08-25〜）がiPhoneで右にはみ出す不具合を修正（2026-09-01）**: `.parking-dashboard`のグリッドが、モバイル用（1列）の定義だけ`grid-template-columns: 1fr`（`minmax(0, 1fr)`無し）のままだったのが原因。グリッド項目（カード）の暗黙の最小幅が中身（ランキング行の折り返さないラベル等）のmin-content幅になり、画面より広がってしまう典型的な不具合で、PC幅（1024px以上）の3列表示は2026-08-25に既に同じ理由で`minmax(0, 1fr)`へ修正済みだったが、モバイル側は直し忘れていた。モバイル側にも`minmax(0, 1fr)`を適用し、カード自身にも`min-width: 0`を明示して二重に防いだ（SafariはChromiumと入れ子のflex/grid間での縮小可否の伝播が異なることがあるため）。**依頼元が実機で確認しOKとの回答済み**
- **撮影・写真選択の直後に拡大表示へ自動遷移（2026-09-01）**: `ReportParkingViolations.jsx`（日報詳細内の登録UI）・`ParkingViolationDetail.jsx`（一覧の明細モーダル）双方の写真アップロード処理は、成功後に一覧へ追加するだけで拡大表示（`preview`状態）を開いていなかった。アップロード成功直後に`setPreview(...)`を呼ぶよう変更し、撮影・選択した写真がそのまま拡大表示で開き、続けて「写真から読み取る」（上記のAI読み取り）へ操作を流せるようにした。**依頼元が実機で確認しOKとの回答済み**
...
- **測定施設**: `BKB`（＝備後町コイズミビル）/ `小泉本社` / `旧本社`の3つ（サーバー側の許可リスト `VALID_CHLORINE_BUILDINGS`）。
  当初（2026-08-10）は前2施設のみが対象だったが、2026-08-25にFileMakerの残留塩素データ520件を全件移行した際、
  移行元に3施設目「スイングビル」の実測データ154件が含まれ継続測定中と判明したため、`CHLORINE_BUILDINGS`
  （フロント）・`VALID_CHLORINE_BUILDINGS`（サーバー）の両方に追加してフル対応した。**同日中に呼称が
  「旧本社」に変更された**ため、上記2箇所の定数と既存データ（`chlorine_tests.building`）をあわせて
  `スイングビル`→`旧本社`に置き換え済み（詳細はHANDOFF.md 155・158番）
- **3施設の月次推移グラフ（`ChlorineChart.jsx`の`ChlorineTrendChart`。2026-08-25〜）**: 画面上部に過去3年・
  施設別の月平均濃度をSVG折れ線グラフで表示する。凡例は必ず表示（2系列以上のためdataviz非交渉事項）、
  水道水質基準（0.1mg/L）の目安線を破線で表示、Y軸は0.1mg/L刻みの全グリッド線にラベル、月ラベルは3年分（36件）が
  詰まらないよう毎年1月・6月のみ表示する。**Y軸ラベルはプロット領域の外側（左）に配置**し、折れ線・マーカーと
  重ならないようにしている（2026-08-25〜26に実機フィードバックを受け、目盛り線の追加→濃さ調整→点線から実線化
  →全グリッド線へのラベル付与→ラベルを外側へ、の順に5ラウンド調整済み。詳細はHANDOFF.md 156・157番）
...
「登録していつでもダウンロードできる資料置き場」。ハンバーガーメニュー・ダッシュボード・PC幅の業務メニュー（4-3参照）の「雛形ファイル」（`/documents`）から入る。日報等の業務データとは無関係のプロジェクト共通情報（`closed_days`と同じ扱い）。
...
- **原本とPDF版の2ファイル登録（2026-08-30追加）**: 「原本（Word/Excel等）とPDF版の2つの物理ファイルを登録できるようにしてほしい。原本がPDF以外の場合にPDFを生成して登録できるか」との依頼。
  ...
  - **登録画面**: 原本を選択し、かつ原本がPDF以外のときだけ「PDF版（任意）」の2つ目のファイル欄を表示する。あとから登録することもできる旨を案内。**ファイル選択時、資料名称欄が空ならファイル名（拡張子を除く）を自動入力する**（2026-08-30・193番。`DocumentTemplateForm.jsx`の`handleFileChange()`。既に手入力済みの名称は上書きしない）
- **一覧のレイアウト（2026-08-30に2行組みへ全面刷新。193番でファイル情報の表示位置・書式を再調整）**: 1件を2行で表示する。
  - **1行目**: 資料名称 ／ 原本ボタン（黒・角丸「↓ {拡張子}」）／ PDFボタン（青・角丸「🔍 PDF」）／ 削除（ゴミ箱アイコン）。**ファイルサイズ・更新タイムスタンプはボタンの真下に`display: block`で1行表示**（193番。当初はボタン右横の`inline`表示だったが、`更新日`表示を`formatDateTime()`によるフルタイムスタンプ表示に変更したのに伴い、横に並べると折り返してしまうためボタン下の独立行に変更。列幅（`table-layout: fixed`のnth-child指定）も資料名称26%／原本34%／PDF30%／削除10%に広げ、`white-space: nowrap`のまま1行に収まるようにした）
  ...
  - **原本ボタン＝常に直接ダウンロード**（`downloadDocument(id, filename, 'original')`。プレビューは試みない）。**PDFボタン＝アプリ内プレビュー**（`AttachmentPreview.jsx`。既存の添付ファイルプレビュー・自主検査表PDFと同じ「短時間有効な署名トークン＋実URLへの`<iframe>`ナビゲーション」方式。`multi-env-attachment-preview`スキル参照）。アイコンは**書類の形をした`IconDocument`**（`Icons.jsx`。2026-09-01追加。203番）に統一している（下記参照）
  - **プレビュー系ボタンのアイコンを統一（2026-09-01。203番）**: 「開いて見る」操作（アプリ内プレビュー・帳票のPDF出力）と「そのままダウンロード」操作を区別するため、新設した`IconDocument`（折れた角を持つ書類の形）を、雛形ファイルのPDFボタン（元は`IconSearch`）・備品の「伝票出力」ボタン（元はアイコン無し。`equipment-plan.md`参照）・日報一覧の「自主検査」「会議室予約表」ボタン（元は`IconDownload`）の4か所に適用した。原本ダウンロード・写真ダウンロード等の**純粋なダウンロード操作は`IconDownload`のまま**で変更していない
...
- **サムネイル自動生成（`src/lib/pdfThumbnail.js`。`pdfjs-dist`使用）と`pdfjs-dist`のバージョン固定について（2026-08-31・196番）**: PDFの1ページ目をブラウザ側で縮小してサムネイルにする際、`pdfjs-dist@6.x`系（`6.0.227`〜`6.3.289`で確認）には2つの罠がある。**①**`getDocument({data}).promise`で得られる`PDFDocumentProxy`には`.destroy()`メソッドが実際には存在しない（型定義上は存在することになっているが実装に無い）。後始末を`pdf.destroy()`で行うと`finally`節で例外が発生し、**レンダリング自体は成功していたのに例外に上書きされて「失敗」扱いになる**（JSの`finally`内例外は`try`の戻り値を上書きする挙動のため）。正しくは`getDocument()`が返す`loadingTask`（`.promise`で解決される前のオブジェクト）側の`.destroy()`を呼ぶ。**②**それとは別に、`6.x`系は内部の随所で標準化前の`Map.prototype.getOrInsertComputed`を無条件に使っており、これが無い実行環境（動作確認した範囲では現行の主要ブラウザの多くが該当）では①を直してもなお失敗する。**このため`pdfjs-dist`は`5.4.624`に固定（`package.json`で`--save-exact`）している。将来的にバージョンを上げる場合は、この2点（`destroy()`の呼び出し先、`getOrInsertComputed`の使用有無）を必ず確認すること**

### 4-16. 連絡帳（顧客の連絡先台帳。2026-09-01〜）

「タスクやメールの取得実績より、顧客の連絡帳を自動作成してほしい」との依頼で新設。ハンバーガーメニュー・ダッシュボード・PC幅の業務メニューの「連絡帳」（`/contacts`）から入る。タスク・メールから抽出した他社の顧客情報を扱うため、owner・備品出庫限定ロールには**表示しない**（4-15の雛形ファイルはowner等も閲覧できるが、連絡帳はそれより一段厳しい`RequireStaff`）。
...
- **管理項目**: 会社名（必須）・業務分類（自由入力＋既存値からの候補選択。まだ運用が固まっていないため選択式には強制していない）・顧客担当者名・当社の主担当者（`settings.assignees`を選択肢にした`<select>`）・会社電話番号・携帯電話番号・メールTO・メールCC（連絡の際に定例で付く先。カンマ区切りで複数可）・ホームページアドレス
  - **現状の投入状況（2026-09-16時点の本番。21件）**: `email_to`は21件すべてに入っているが、**会社電話番号・携帯電話番号・メールCC・ホームページアドレスは0件**。連絡帳が「タスク・メールの取得実績からの自動作成」で育っており、そこから取れるのはアドレスだけのため。したがって**電話・地球儀のアイコンは、番号やURLを手で入れるまで出てこない**（空の項目にはアイコンを出さない仕様のため）
...
- **mailto:／tel:リンク**: 一覧の各行に会社電話・携帯電話（登録があるものだけ）・メール・ホームページのアイコンボタンを表示する。メールは`src/lib/contacts.js`の`buildContactMailto()`が`mailto:{email_to}?cc={email_cc}`を組み立て、新規メール作成画面を開くと同時に登録済みのCCを自動付与する（返信ではなく新規作成のため件名・本文は入れない）。電話は`tel:{番号}`のリンクで、モバイルではダイヤラーが起動する。**会社電話は`IconPhone`（受話器）・携帯電話は`IconMobile`（スマートフォンの筐体）**と別のアイコンにしている（2026-09-16。当初はどちらも受話器で、一覧で同じアイコンが2つ並んで分かりにくいとの指摘による）
  - **編集モーダルにも同じ導線を置いた（2026-09-16。依頼）**: 連絡先を開いて中身を見ている状態からそのまま掛けられるよう、**各項目の見出しの右**にアイコンを並べた。会社電話番号・携帯電話番号＝電話（`tel:`）、メール＝メール作成（`mailto:`）、ホームページアドレス＝地球儀（別タブ＝アプリ外のブラウザで開く）。**リンク先は保存済みの値ではなく「今入力されている値」から作る**ので、書きかけの番号でも保存せずに試せる（値が空の項目にはアイコンを出さない＝押しても何も起きないボタンを作らない）
  - **メールはTO・CCで見出しを分けず「メール」1つにまとめ**、入力欄の左に`TO:`／`CC:`を添える形にした（メール作成は1つの操作で、TOに送りCCを自動で付ける、という関係が見た目で分かるようにするため）。従来の見出し「メールTO」「メールCC」はこの形に置き換わっている
  - **`toExternalUrl()`（`src/lib/contacts.js`）**: ホームページは`www.example.com`のようにスキームの無い値が入りうる。そのまま`href`に入れると**アプリ内の相対パス**として解釈されて開けないため、スキームが無ければ`https://`を補う。`javascript:`等、http(s)以外のスキームは`null`を返して**アイコン自体を出さない**。一覧側のリンクも同じ関数を通すようにした
- **画面構成**: `src/pages/Contacts.jsx`＋編集モーダル`src/components/ContactForm.jsx`。備品マスタ・雛形ファイル一覧と同じ`.ui-page`/`AppHeader`/`FeatureHeader`/`.ui-table`の標準レイアウト。検索対象は会社名・業務分類・顧客担当者名・主担当者・メールTOの部分一致（クライアント側フィルタ）。行クリックで編集モーダルを開く（`EquipmentItemForm`等と同じ`ui-overlay`/`ui-modal`標準フォーム）
  - **検索窓は常時表示（2026-09-01。203番）**: 他画面（違反車両一覧・日報一覧）は虫眼鏡タップで検索欄を開閉する方式だが、連絡帳は一覧の主要な操作が検索のため、`FeatureHeader`の`filters`側（`.ui-feature-head-main`）に検索窓を常時表示する。PC幅では検索窓・「連絡帳を自動作成」・「連絡先を追加」が同じ行の右端まで1行に収まる
  - **モバイル幅ではボタンを整理（2026-09-01。203番）**: 「連絡先を追加」はキャプション付きボタンではなく「＋」アイコンボタン（`.contacts-add-icon-btn`。`.icon-btn-add`）に省略し、「連絡帳を自動作成」はハンバーガーメニューの業務別メニュー（モバイル幅限定。`AppHeader.jsx`の`onContactSync`/`contactSyncBusy`プロップ）へ移す。PC幅ではどちらも従来どおりヘッダーのボタンから実行できる
  - **業務分類でグルーピング表示（2026-09-02。205・206番）**: 一覧は`filtered`（検索適用後）を`business_category`ごとにグルーピングし、備品マスタ画面（4-6等）と同じ「`colSpan`の見出し行を`<tbody>`内に差し込む」方式で表示する（業務分類が未設定の連絡先は「（業務分類未設定）」としてまとめる）。見出し行は列を1つ減らす（会社名／顧客担当者名／主担当者／連絡先の4列。業務分類は見出しに出るため個々の行からは削除）。見出しの帯は共通部品化した`.ui-table-group-head`（`src/styles/ui.css`。備品マスタの`.equipment-master-group-head`から改称・統合）を使う。**当初は`--bg-subtle`（表の地の白とほぼ差の無い淡いグレー）のままだったため区切りが視認できないとの指摘を受け、ブランド色を薄く敷いた帯（`--color-brand-soft`）＋濃い太字の文字色に変更した（206番）**。この帯は備品マスタ画面にも共通で効く
- **API**: `GET/POST/PATCH/DELETE /api/contacts`、`GET /api/contacts/suggest`（業務分類の候補）、`POST /api/contacts/sync`（`worker/lib/contacts.js`）。権限は雛形ファイルと同じ`requireAuth`パターンだが、ルート自体がRequireStaffのため実質staff/adminのみが到達する
- **バックアップ**: 日次バックアップは`public`スキーマ全体を対象のため、`contacts`テーブルは設定変更なしで自動的に対象へ入る
- **実機確認の状況（2026-09-01）**: 会社名・担当者名の分離、自社アドレス・FAX誤登録の解消は依頼元が確認済み。**依頼元は「しばらく様子を見る」段階**（自動作成を運用しながら誤登録が無いか継続確認中）
  - **2026-09-16**: 編集モーダルの発信アイコン・携帯電話アイコンの変更も依頼元が確認済み（「OKです」）。ただし前述のとおり電話番号・ホームページは本番に1件も入っていないため、**実際に押せたのはメールアイコンだけの可能性が高い**。実データが入ってから電話・地球儀を改めて見てもらうとよい

### 4-17. リマインダー（システム運用上の期限管理。2026-09-07〜）

「Anthropic APIキーの更新のような、期限のある運用作業を、いざその時に内容・目的・手順まで
思い出せる自信がない」との依頼で新設。ハンバーガーメニューの共通領域（全画面共通。連絡帳・
...
- **通知からのジャンプ**: Web Push（`worker/lib/push.js`の`notifyReminder()`）のペイロードに
  ...
  `GET /api/reminders?id=`で単体取得する。`ReportList.jsx`の`/reports/:date`と同じ考え方）。
...
- **日付の曜日表示（2026-09-07追加・同日改修）**: 期限日付・通知タイミングは
  `formatDateWithWeekday()`（`src/lib/format.js`。日付部分と曜日部分を別々に
  `toLocaleDateString('ja-JP', ...)`で組み立てて連結し、常に`yyyy/mm/dd (曜)`の形式で
  固定する）で表示する。一覧はこの文字列をそのままセル表示するだけで済むが、編集
  モーダルの`<input type="date">`はネイティブ部品のため曜日はおろか、この形式そのものも
  表示できない（ブラウザ・ロケール依存の表示になる）。そこで「日付欄の隣に曜日バッジを
  添える」方式は撤回し、`ReminderForm.jsx`の`DateField`コンポーネントで**ネイティブの
  `<input type="date">`を透明化して同じ位置に重ね、見た目は`formatDateWithWeekday()`の
  文字列を表示する自前の`<div>`に差し替える**方式にした（カスタム日付欄の一般的な実装
  手法。タップ・キーボード操作はすべて透明な下のネイティブ入力が受け、見た目の`<div>`は
  `pointer-events: none`で素通しにする）。これにより「フィールド内に`yyyy/mm/dd (曜)`で
  表示してほしい」という依頼どおりの見た目になり、通知タイミングの日付欄が横並びの
  2カラム（`is-halves`）で欠けて表示される不具合も併せて解消した。通知タイミングの
  日付欄と時刻欄は横1行にまとめている（`.reminder-notify-row`。当初は縦2段だったが、
  日付欄がフィールド内表示に変わり幅の制約が緩んだため、依頼により1行へ戻した）。
  狭幅では1回目/2回目自体を縦積みにして各欄にモーダル幅いっぱいを与える
  （`.reminder-notify-fields`。モーダル`.ui-modal.is-sm`の最大幅620pxに対し、
  ビューポート幅がまだそれ未満の561〜600px付近だけ2カラムだと日付欄の文字が
  欠ける実測結果があったため、ブレークポイントは560pxではなく660pxにしてある）
...
- **初期登録データ（2026-09-07）**: システム運用上、現時点で期限が判明している2件を登録済み。
  ①「Anthropic APIキーの更新」（期限2027-09-07。234番で会社アカウントへ移管した際に発行した
  ワークスペーススコープキーの有効期限）、②「年次バックアップ復元ドリル」
  （期限2027-08-27。前回2026-08-27実施分の1年後。`docs/disaster-recovery.md`7章）
- **バックアップ**: 日次バックアップは`public`スキーマ全体を対象のため、`reminders`テーブルは
  設定変更なしで自動的に対象へ入る

### 4-18. 古紙回収（リサイクル用の古紙回収量。2026-09-08〜）

備後町コイズミビル（BKB）で毎週行っている古紙回収の計量記録。廃棄物実測値管理（`docs/waste-plan.md`。
このスペック上に専用の章は無い）と対になる機能で、従来はExcel「月別古紙回収量一覧表」（年度ごとのシート・毎週月曜の行）で
記録していたものを、そのままの見え方でこのシステムへ移した。画面は`/paper`
（`src/pages/Paper.jsx`）。権限は廃棄物と同じ（owner・備品出庫限定ロールは閲覧のみ）。

- **記録の単位**: 1回の回収につき1行。区分は**段ボール／シュレッダ／雑誌／その他**の4つで、
  合計は保存せず都度計算する（Excelの合計列と同じ）。単位はkg
- **回収予定日はDBに持たない**: 回収は原則毎週月曜のため、年度内（4月〜翌3月）の月曜を
  画面側で自動生成して行として並べ（`collectDatesOfFiscalYear()`。`src/lib/paper.js`）、
  **1件でも入力した回だけ**を`paper_records`に保存する。Excelの「回収予定日（自動）」列と
  同じ考え方で、未入力の週のために空行をDBへ作らない。日程変更で月曜以外に回収した記録が
  DBにあれば、その日付も行として差し込むため、予定日から外れた記録も隠れない
- **祝日・休館日で中止になる週への対応（依頼元の運用）**: 回収は祝日・休館日と重なると
  中止になったり日がずれたりする。①中止した週は`skipped`を立てて合計・月平均から外す、
  ②日がずれた週は**回収予定日そのものを直接書き換える**（後述の「日付の直接編集」）、
  という2本立てにした。あわせて、その週が中止になりやすいことが一目で分かるよう、
  **祝日名（`GET /api/report/holidays`）と休館日（`closed_days`。4-12と共通）を日付欄にバッジ表示**する
- **日付の直接編集（2026-09-09。依頼）**: 当初は日程がずれた週を備考への自由記入
  （例:「火曜に変更」）で扱っていたが、実際の日付が記録に残らないとの依頼で、
  **回収予定日のセル自体を編集可能にした**。透明化した`<input type="date">`を重ね、
  見た目は自前の文字列（年を省いた表示・祝日/休館日バッジ込み）を出す
  （`ReminderForm.jsx`の`.reminder-date-field`と同じ技法）。変更すると、元の日付にあった
  記録（重量・中止・備考）を新しい日付へ移し、元の行は削除する（回収予定日はDBに持たないため、
  移動元は自動生成の空行に戻る）。**移動先の日付に既に記録がある場合は変更を拒否する**
  （上書きによるデータ消失を防ぐため）。この直接編集ができるようになったことで、
  **右側の欄（旧「日程変更・備考」）は日付変更の手段ではなく純粋な備考欄になったため、
  見出しを「備考」だけに変えた**。**見た目は他の入力欄（`.paper-note-input`等）と同じ
  枠付きにして、日付欄も編集できることが一目で分かるようにした**（2026-09-09。
  「編集できることを確認したが視覚的に分かりにくい」との指摘）。この対応中に、
  `.paper-note-input`・`.paper-weight-input`が参照していた`--border-subtle`という
  CSS変数がどこにも定義されておらず、**入力欄の枠線がずっと表示されていなかった不具合**に
  気付き、実在するトークン`--border-strong`（他画面の`.ui-input`と同じ）に差し替えた
  （Paper.css内の4箇所のみ。他ページはこの変数を使っていない）
- **祝日名は表示せず日付を赤字にする（2026-09-09。依頼）**: モバイルでの横幅節約のため、
  祝日名のバッジ表示をやめ、`weekdayInfo()`が返す`is-holiday`/`is-saturday`のclassNameで
  日付の文字色を赤（日曜・祝日）／青（土曜）にする（Waste.css等、他の日付列と同じ配色）。
  祝日名自体は`aria-label`に含めてスクリーンリーダーからは分かるようにしている
- **表のコンパクト化（2026-09-09。依頼で列構成を全面的に組み直した）**:
  - **休館日バッジも撤廃**（前項の祝日名撤廃に続き、こちらも表記を省いてさらに横幅を節約）
  - **`table-layout: fixed`に変更**。以前は各列の幅がブラウザの自動計算任せ（`table-layout: auto`）
    だったため、入力内容やチェック状態で列幅がわずかに変わり、表自体が左右に揺れて見える
    ことがあった（依頼原文「表自体が左右にぐらつく」）。全列に`width`を固定指定することで
    解消したが、**`.ui-table`の`width: 100%`を打ち消して`width: auto`にするのが必須**
    （`table-layout: fixed`でも表自身の幅が100%のままだと、各列のpx指定は絶対値ではなく
    比率として扱われ、広い画面ほど列が間延びする）。さらに`width: auto`だけでは狭い画面で
    逆に全列が比例縮小されてしまうため、**全列幅の合計と同じ値を`min-width`にも指定**し、
    どの画面幅でも縮まないようにしている（余った分は表のラッパーの横スクロールに任せる）。
    実装中、`<th>`に直接`display: flex`を当てると`table-layout: fixed`の幅指定が無視される
    という挙動を踏んだため、フレックスは`<th>`直下のラッパー要素（`.paper-th-date-row`）に
    持たせ、`<th>`自体は素の`table-cell`のままにしてある
  - **「中止」を回収予定日のセルへ統合**: 従来は備考欄の左に「中止」ラベル＋チェックボックスが
    あったが、回収予定日のセルの右端へ移した。見出し行はラベル「中止」を表示し、明細行は
    ラベル無しのチェックボックスのみ（見出し側にラベルがあるため）。日付欄自体は
    セルの左端に詰める（中止チェックボックスとの間の`justify-content: space-between`で
    左右に振り分ける形にしたため、以前の字下げ用の左余白は不要になった）
  - **備考欄を表の右端に移動**: 中止チェックボックスと同居させる理由が無くなったため独立させ、
    合計列の右（＝表の最後尾）に置いた
  - **重量入力欄を3桁幅に縮小**: 実測値は999kgが上限のため、以前の5.5em幅は過剰だった。
    列見出し（「シュレッダ」等）の文字幅の方が実測値の3桁より広いため、実際の列幅は
    見出し文字が収まる最小幅（90px）で決まっている。見出しは文字サイズ・左右余白を
    小さくして、この幅に収まるようにしている
- **月平均は「実績のある月」だけで割る**: Excelが未到来の月をあえて`#N/A`にして平均・近似曲線から
  外しているのと同じ考え方で、合計が0の月は分母に数えない（画面には「月平均（実績Nか月）」と
  分母を明示する）
- **表示**: 上部に年度合計・月平均と月別合計の棒グラフ（Excel右側の月別集計＋グラフに相当。
  専用のグラフ描画ライブラリは入れていないためdiv+CSSで組む）、その下に週ごとの入力表を
  月見出しでグルーピングして並べる。**1列目（回収予定日）は左端に固定**し（frozen column。
  Waste.cssの月別一覧と同じ手法）、区分4列を横スクロールしながら入力しても「どの週の行か」を
  見失わないようにしている
- **列見出しの固定表示と「本日」ボタン（2026-09-09。依頼）**: 年度は52週あり縦に長いため、
  ①スクロールしても列見出し（段ボール／シュレッダ…）が残ること、②目当ての週へ一発で飛べること、
  の2つを足した。①は**表のラッパー自身を縦スクロールコンテナにして、その中で列見出しを固定する**
  方式（`.paper-page`を縦のflexにして`.paper-table-wrap`に残りの高さを渡す）。表のラッパーは
  横スクロール（`overflow-x: auto`）を持ち、CSSの仕様でそこは縦のスクロールコンテナにもなるため、
  ページ基準の`position: sticky`では列見出しを固定できない（`.claude/skills/sticky-header-overflow-trap`）。
  結果として年度サマリ・グラフは常に見えたまま、週の行だけが流れる。②は日報一覧と同じ「本日」ボタンで、
  **本日、または本日に一番近い過去の回収日**（回収は週1回なので水曜に開けば直前の月曜）の行へ
  スクロールする。他年度を見ているときは今年度へ切り替えてから飛ぶ。その行は常に色を付けている
- **日付は「9/7（月）」の形（2026-09-09。依頼）**: 年は月見出し行（「2026年9月」）が示すので省き、
  月見出しの下にぶら下がって見えるよう字下げする。固定列の幅が詰まるぶん、狭い画面で計量値の
  入力欄に幅を回せる
- **API**: `GET/PUT/DELETE /api/paper/records`（`worker/lib/paper.js`）。`GET`は`?fiscal_year=2026`で
  年度内を取得、`PUT`は回収予定日（`collect_date`）をキーにした upsert（1マス直すだけでも
  その回の全項目を送る）
- **過去データの移行（2026-09-08実施）**: 依頼元から共有されたExcel（2016年〜2026年のシート）を
  `scripts/import-paper-records-xlsx.py`で取り込み、**545件（2016-01-05〜2026-08-31。うち中止10件）**を
  投入済み。シートは2形式（2020年〜は1行=1回、2016〜2019年は月×回収日のクロス集計）あり、
  スクリプトは両方を読んで各回の4区分の和がExcel側の合計と一致することを全件検証してから出力する。
  取り込み時に見つかったExcel側の要注意点は次の2つ（**数値はExcelの実績値の方を採用している**）:
  ①「平成２８年度合計」の式が`=SUM(B32:M32)`でN列を含まないため、2016/12/26の207kgが年度合計から
  漏れている（Excel上は11246kg、正しくは11453kg）、②2022-12-26の備考に入っている日付が
  「2023/12/28」と1年ずれている（2022/12/28の誤記と思われるが、判断できないため原文のまま取り込み）
- **バックアップ**: 日次バックアップは`public`スキーマ全体が対象のため、`paper_records`テーブルは
  設定変更なしで自動的に対象へ入る
...
| contact | text | 先方担当者の宛名（会社・氏名＋様）。返信メール冒頭に使う。AI 抽出、無ければ `sender_display`＋様。本番に既存の列。往信・返信どちらでも常に「先方（顧客）」を指す（`sender_display`は「送信元」なので自社発信メールでは自社側の氏名になる点が異なる） |
| contact_company | text | `contact`を構造化: 先方（顧客）の会社名・組織名のみ（敬称『様』は付けない）。連絡帳の自動作成・タスク詳細画面の「顧客」行で使う（マイグレーション `add_tasks_contact_company_person`。2026-09-01追加） |
| contact_person | text | `contact`を構造化: 先方（顧客）の担当者の氏名のみ（マイグレーション `add_tasks_contact_company_person`。2026-09-01追加） |
| sender_email | text | 返信先アドレス（フォーム経由は本文記載を優先） |
| sender_cc | text | 受信メールのCcに入っていた先方アドレス（自社アドレスは除く。カンマ区切り）。連絡帳の自動作成でメールCC欄の初期値に使う（マイグレーション `add_tasks_sender_cc`。2026-09-01追加。以降に取り込むメールから値が入る） |
...
（2026-09-07訂正。当初は設定画面`/settings`へのリンクだったが、上限の入力欄自体は
上記のとおり従量課金事項の画面に置いているため、実際に変更できる画面へ直接飛ぶよう修正した）。
**上限到達時とAPIクレジット不足時はWeb Push通知も送る**（2026-09-04。気づかないうちに自動分類が
止まっていると業務メールを取りこぼすため）。連続送信を防ぐため、上限到達は`api_limit_alert`の
`day`が本日でない時（＝その日の最初の1回）、クレジット不足は`api_credit_alert`が空だった時
（＝残高が尽きた最初の1回）だけ送る。
