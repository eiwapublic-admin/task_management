...
**結論: Cloudflare R2 を採用する。**

R2 の無料枠は「ストレージ 10GB/月・Class A（書込系）100万リクエスト/月・Class B（読取系）1000万リクエスト/月・
egress 無料」。本システムの想定使用量と比べると桁違いに小さく、超過の心配は実質ない。

| 区分 | 想定使用量（月） | 無料枠（月） | 使用率 |
|---|---|---|---|
| Class A（アップロード） | 54 回 | 1,000,000 回 | 0.005% |
| Class B（閲覧） | 1,338 回 | 10,000,000 回 | 0.013% |
| ストレージ | +31 MB | 10 GB | 10GB 到達まで約27年 |

> **現在の状態（2026-08-04 に API で確認）**: R2 は**未有効化**。
> `r2_buckets_list` が `403 code 10042: Please enable R2 through the Cloudflare Dashboard` を返す。
> Cloudflare ダッシュボードの「R2 object storage」から有効化する必要がある。
> 有効化の手続きで支払い方法の登録を求められる場合があるが、上表のとおり無料枠内に収まるため課金は発生しない。
...
| 写真ストレージ | Cloudflare R2 を採用（有効化手続きは下記） |

### 残る確認点

1. **Cloudflare R2 の有効化** — API で確認したところ**未有効化**（`code 10042: Please enable R2 through the Cloudflare Dashboard`）。
   ダッシュボードの R2 から有効化する。無料枠は「ストレージ10GB/月・Class A 100万/月・Class B 1000万/月・egress 無料」で、
   本システムの想定使用量は Class A が月54回（無料枠の 0.005%）、Class B が月1,338回（同 0.013%）、
   ストレージは 10GB 到達まで約27年と、いずれも無料枠に対して桁違いに小さい
2. **過去データの移行範囲** — 不正駐車は累計回数の連続性のため移行を推奨。作業記録・写真はどうするか
   （移行しない場合は現行 FileMaker を参照用に残す）
3. **動画（mov）の扱い** — 容量が写真の約4倍。上限サイズ・保管期間を設けるか
4. **FileMaker Server の外部到達性** — 方式B（Pull）を採る場合のみ必要。方式A（Push）なら確認不要
