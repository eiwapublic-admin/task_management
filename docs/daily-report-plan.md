...
`--reports-header-bg` / `--reports-header-ink`）とし、タスク一覧のグリーン系と区別する。
...
- 未入力の日付行には「＋」（点検データを追加）と「休館日」（その日を休館としてグレー表示）の2ボタンを配置。
  休館日にした行には誤操作対策として「休館取り消し」ボタンを表示する（実体は通常の削除と同じ）
...
| ボタン・アイコンの標準 | 1-4章のとおり。大きめ・アイコンベースに統一し、既存の「タスク管理標準」の配色に合わせる |
| 自主検査表の曜日・祝日表示 | 土曜＝青字、日曜・祝日＝赤字。祝日データは Worker 経由で取得しキャッシュ |
| 自主検査表の休館日 | 「＋」で点検追加、「休館日」でグレー表示＋「休館取り消し」で復元。右上の固定ボタンは廃止 |
| 自主検査表・定型文の設定の導線 | ハンバーガーメニューから日報一覧画面のツールバーへ移動 |
