...
> **このプロジェクトでの注意（2026-08-06 導入）**: 他プロジェクトで作成したスキルをそのまま
> 移植したもの。`task_management` は Vite + React（Next.js ではない）で、CSS も自前の
> カスタムプロパティ（`--color-primary` 等）を使う手書きCSSであり **Tailwind は使っていない**
...
**2026-09-09 follow-up — split the single "Share / Save" button into two explicit buttons.** The
combined button above fixed the blank-screen crash, but real usage surfaced a second-order UX bug:
tapping it always tries Web Share first, so on any device that supports it the OS share sheet opens
even when the user's actual goal is "save this to my device" — and picking "Save to Files" from that
sheet is a second tap the user didn't expect, with no visible "just download it" option at all. The
fix is to stop trying to guess intent from one button: render a **"ダウンロード" button that always
runs the plain blob-download path (never Web Share)**, plus a separate **"共有…" button, shown only
when `navigator.canShare` reports support**, that calls `navigator.share` and falls back to the same
download path only if the user picks a target and the OS call itself fails (an `AbortError` from the
user dismissing the sheet is still not a failure). See `src/components/PdfPreviewActions.jsx` for the
shared implementation — every PDF-preview hook in this app (`useInspectionPdfExport.jsx`,
`useChlorinePdfExport.jsx`, `useBilmenNoticePdfExport.jsx`, etc.) renders it as the `headerAction`
passed to `AttachmentPreview` instead of keeping its own local `sharePdf` copy, so this fix — and any
future one like it — only needs to happen in one place.
